 ...
 * @param {function} [options.errorCallBack]
 */
function showPermanentlyDeletePopupModal(url, options = {}, message = null) {

    // To Preserve OLD
    let opt = {
        title: window.trans["Are you sure"],
        text: message ? message : window.trans["deleting_this_will_remove_the_data_and_all_connected_information_across_all_session_years_this_action_cannot_be_reversed"],
        icon: 'error',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
const getFilterSubjectOptionsListClassWise = (SubjectId, $this) => {
    $(SubjectId).val("").removeAttr('disabled').show();
    $(SubjectId).find('option').hide();
    if ($this.val()) {
        if ($(SubjectId).find('option[data-class-id="' + $this.val() + '"]').length) {
            $(SubjectId).find('option[value=""]').show();
            $(SubjectId).find('option[data-class-id="' + $this.val() + '"]').show();
        } else {
            $(SubjectId).val("data-not-found").attr('disabled', true).show();
        }
    } else {

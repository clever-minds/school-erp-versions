...
// Reapeater On Lesson And Topic Files
var IdLessonTopicCounter = 0;
var addNewLessonTopicFileRepeater = $('.files_data').repeater({
    initEmpty: true,
    show: function () {
        // Remove Button
...
select2Search($(".edit-school-admin-search"), baseUrl + "/schools/admin/search", null, 'Search for school admin Email', Select2SearchDesignTemplate, function (repo) {
    if (!repo.text) {
        $('#edit_admin_email').val(repo.email);
...
select2Search($(".guardian-search"), baseUrl + "/guardian/search", null, 'Search for Guardian Email', Select2SearchDesignTemplate, function (repo) {
    if (!repo.text) {
        ...
        $('#guardian_first_name').val(repo.first_name);
        $('#guardian_last_name').val(repo.last_name);
        $('#guardian_mobile').val(repo.mobile);
        if (repo.gender == 'male') {
        ...
        $('#guardian-image-tag').attr('src', repo.image);
    } else {
...
const electiveSubjectGroupRepeater = $('.elective-subject-group-repeater').repeater({
    initEmpty: true,
    show: function () {
        // console.log('outer show function');
        $(this).slideDown();
    ...
    hide: function (deleteElement) {
        if ($(this).hasClass('elective-subject-group')) {
            // console.log("outer hide function");
    ...
    repeaters: [{
        ...
        hide: function (deleteElement) {
            // console.log('inner hide function called');
...
var examTimetableRepeater = $('.exam-timetable-content').repeater({
    initEmpty: true,
    show: function () {
        let $this = $(this);
        $this.slideDown(); // Add Another Data Down
...
$('#exam-id').on('change', function () {
    getExamSubjectOptionsList('#class_subject_id', $(this))
});

var addNewOptionRepeater = $('.options-data').repeater({
    initEmpty: true,
    show: function () {
...
var addNewClassFeesData = $('.class-fees-data').repeater({
    show: function () {
        $(this).slideDown();

        // If Fees Class ID is present then add id or else set null
        let id = $(this).find('#fees-class-id').val()
        $(this).find('.remove-fees-data').attr('data-id',id);

        // Check the duplicate Values in Fees Type Select Option
        var feesTypeElement = $(this).find('.fees_type');
        feesTypeElement.each(function (index, element) {
            $(element).rules("add", {
                "noDuplicateValues": {
                    parentClass: "class-fees-data",
                    class: "fees_type",
                    value: feesTypeElement.find("option:selected").text()
                }
            });
        })
    ...
    hide: function (deleteElement) {
        let feesClassId = $(this).find('.remove-fees-data').data("id");
        if (feesClassId) {
            let url = baseUrl + '/fees/remove-fees-class/' + feesClassId;
            showDeletePopupModal(url, {
                successCallBack: function () {
                    $(this).slideUp(deleteElement);
                    $('#table_list').bootstrapTable('refresh');
...
// Event on close Fees Class Modal and make FeeClassIdCounter to 0
$('#closeFeesClassModal').on('click', function () {
    feeClassIdCounter = 0;
})


var addNewInstallmentData = $('.fees-installment-content').repeater({
    show: function () {
        $(this).slideDown();
...
toggleContentAccordingToValue('.fees-installment-toggle', '.fees-installment-content')               // Toggle Fees Installement Content
toggleContentAccordingToValue('.edit-fees-installment-toggle', '.edit-fees-installment-content')     // Toggle Edit Fees Installement Content


var addNewEditInstallmentData = $('.edit-fees-installment-content').repeater({
    show: function () {
        $(this).slideDown();

        $(this).find('.edit-installment-name').rules("a10/11/2023dd", {
            "noDuplicateValues": {
                class: "edit-installment-name",
                value: $(this).find('.edit-installment-name').text()
            }
        });
        ...
        // Store InstallmentID in Remove Installment Button as Data Attribute
        var installmentId = $(this).find('.installment-id').val();
        $(this).find('.remove-edit-installment-fee').attr('data-id', installmentId ?? "");
    },
    hide: function (deleteElement) {
        let installmentId = $(this).find('.remove-edit-installment-fee').data("id");
        if (installmentId) {
            let url = baseUrl + '/fees/remove-installment-data/' + installmentId;
            showDeletePopupModal(url, {
                successCallBack: function () {
                    $(this).slideUp(deleteElement);
                    $('#table_list').bootstrapTable('refresh');
...
$(document).on('click', '.pay-in-installment', function () {
    if ($(this).is(':checked')) {
        ...
        $(document).find('.due-charges-whole-year').hide(200)
        ...
        $(document).find('.is_fully_paid').val(0)
    } else {
        $('.added_price').trigger('click').addClass('installment-chkclass').removeClass('added_price') // Remove Extra Class and add Old Installment Class to Installment Checkbox
        $(document).find('.mode-container').show(200);
        $(document).find('.due-charges-whole-year').show(200)
        $('.installment_rows').hide(200);
        $('#installment-mode').val(0)
        $(document).find('#is-fully-paid').val(1)
        updateTotalAmount(parseInt($(this).data("base_amount")));
        $(document).find('.compulsory-fees-payment').prop('disabled', false);
    }
})
...
$("#stream_id").on("select2:selecting", function (e) {
    ...
    setTimeout(function () {
        $("#" + e.params.args.data.text + "-section-div").slideDown(500);
    }, 3)
...
$('#stream_id').on("select2:unselecting", function (e) {
    ...
    setTimeout(function () {
        $("#" + e.params.args.data.text + "-section-div").slideUp(500);
    }, 3)
});
...
$('#change-roll-ckh-settings').on('click',function(e){
    Swal.fire({
        title: window.trans["Are you sure"],

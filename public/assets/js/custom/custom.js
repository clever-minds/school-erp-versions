...
$(function () {
    ...
    if ($('#tinymce_message').length || $('.tinymce-editor').length) {
        tinymce.init({
            directionality: layout_direction,
            height: "500",
            selector: '#tinymce_message, .tinymce-editor',
            relative_urls: false,
...
if (calendarEl !== null) {
    var createTimetable = new FullCalendar.Calendar(calendarEl, {
        ...
        eventReceive: function (event) {
            ...
            // Clipping logic
            let slotMaxTime = createTimetable.getOption('slotMaxTime');
            if (slotMaxTime && slotMaxTime !== "00:00:00") {
                let maxTimeParts = slotMaxTime.split(':');
                let maxTimeDate = new Date(end_time);
                maxTimeDate.setHours(parseInt(maxTimeParts[0]), parseInt(maxTimeParts[1]), parseInt(maxTimeParts[2] || 0), 0);
                if (end_time > maxTimeDate) {
                    end_time = maxTimeDate;
                }
            }

            let startTime24Hr = date.getHours().toString().padStart(2, '0') + ":" + date.getMinutes().toString().padStart(2, '0') + ":" + date.getSeconds().toString().padStart(2, '0');
            let endTime24Hr = end_time.getHours().toString().padStart(2, '0') + ":" + end_time.getMinutes().toString().padStart(2, '0') + ":" + end_time.getSeconds().toString().padStart(2, '0');
        ...
        eventDrop: function (event) {
            ...
            data.append('end_time', endTime24Hr);
            data.append('_method', 'PUT');
            ajaxRequest('POST', baseUrl + '/timetable/' + timetable_id, data, null, function (response) {
                event.event.setEnd(end_time);
            }, function (response) {
                event.revert();
    ...
    var teacherTimetable = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDidMount: function (event) {
            let classSection = event.event.extendedProps.class_section;
            let subject = event.event.title;
            let startTime = event.event.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            let endTime = event.event.end.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            let fullText = startTime + ' - ' + endTime + '<br>' + subject + '<br>' + classSection;

            // Add tooltip attributes
            $(event.el).attr('title', fullText)
                .attr('data-toggle', 'tooltip')
                .attr('data-placement', 'top')
                .attr('data-html', 'true');

            // Initialize tooltip with body container to prevent clipping
            try {
                $(event.el).tooltip({ container: 'body' });
            } catch (e) {
                console.error("Tooltip initialization failed", e);
            }

            // Append truncated text with explicit width constraints
            if (classSection) {
                // Ensure parent has hidden overflow to contain the new element
                $(event.el).find(".fc-event-main").css({ 'overflow': 'hidden' });

                $(event.el).find(".fc-event-main .fc-event-main-frame .fc-event-title-container")
                    .append("<div class='mt-1' style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; display: block;'>" + classSection + "</div>");
            }
        },
    })
...
$('#filter-class-id').on('change', function () {
    getFilterSubjectOptionsListClassWise('#filter-subject-id', $(this))

    // getSchoolFilterSubjectOptionsListWise('#school-filter-subject-id', $(this))
});
...
$('#edit_to_date,#edit_from_date').change(function (e, leave_detail) {
    if (leave_detail) {
        ...
        $(div).slideDown(500);
        $.each(leave_detail, function (index, value) {
            let date = moment(value.date, 'YYYY-MM-DD');
            let day = days[date.toDate().getDay()];
            let dateStr = date.format('DD-MM-YYYY');
            let disabled = 'disabled';
            html += '<div class="form-group col-sm-12 col-md-12">';
            html += '<label class="mr-2">' + date.format(jsReplacedDateFormat) + '</label>-';
            html += '<label class="ml-2">' + day + '</label>';
            html += '<div class="form-group row col-sm-12 col-md-12"> <div class="form-check mr-3"> <label class="form-check-label"> <input type="radio" class="form-check-input" name="type[' + dateStr + '][]" id="optionsRadios1" value="Full"> ' + window.trans['full'] + ' <i class="input-helper"></i></label> </div> <div class="form-check mr-3"> <label class="form-check-label"> <input type="radio" class="form-check-input" name="type[' + dateStr + '][]" id="optionsRadios2" value="First Half"> ' + window.trans['first_half'] + ' <i class="input-helper"></i></label> </div> <div class="form-check mr-3"> <label class="form-check-label"> <input type="radio" class="form-check-input" name="type[' + dateStr + '][]" id="optionsRadios3" value="Second Half">' + window.trans['second_half'] + ' <i class="input-helper"></i></label> </div> </div>';
            html += '</div>';
        });
...
$('#filter_class_id').on('change', function () {
    ...
    // If class_id is empty, reset the class_section_id filter and disable it
    if (!class_id) {
        $('#filter_class_section_id').html('<option value="">Select Class Section</option>').prop('disabled', true).val('');
        return; // Exit early, no need to send AJAX request
    }

    // Enable the class section dropdown when class is selected
    $('#filter_class_section_id').prop('disabled', false);
...
$('#change-order-school-form-field').click(async function () {
    ...
    $.ajax({
        type: "post",
        url: baseUrl + "/schools/custom-field/update-rank",
        data: {
            ids: ids
        },
        dataType: "json",
...
$('.remove-existing-fee').on('click', function () {
    ...
    let $btn = $(this);
    $.ajax({
        url: baseUrl + '/pickup-points/delete-transportation-fees/' + id,
        type: 'DELETE',
...
function initAttendanceSection({
    ...
    function renderMonth() {
        ...
        loadAttendanceData(month, year, mode, isStaff);
    }

    // ===== Data Fetch =====
    function loadAttendanceData(month, year, mode, isStaff = false) {
        ...
        fetch(fetchUrl, {
            ...
            .then(data => {
                if (data.message === "No user selected") {
                    ...
                    renderAttendanceTable(data, month, year, isStaff);
                }
            })
            .catch(() => {
    ...
    // ===== Table Renderer =====
    function renderAttendanceTable(data, month, year, isStaff = false) {
        ...
        let url = '';
        if (isStaff) {
            url = baseUrl + '/reports/teacher/teacher-view-reports/';
        } else {
            url = baseUrl + '/reports/student/student-view-reports/';
        }
        /* ------------------------------
           LOOP USERS (correct)
        ------------------------------- */
...
// reset-class-section-data
$(document).on('click', '.reset-class-section-data', function (e) {
    e.preventDefault();

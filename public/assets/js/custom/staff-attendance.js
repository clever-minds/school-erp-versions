$(document).ready(function () {
    ...
    if (sessionYearStart) sessionYearStart.setHours(0, 0, 0, 0);
    if (sessionYearEnd) sessionYearEnd.setHours(0, 0, 0, 0);

    // Clamp initial currentDateObj within session year if needed
    if (sessionYearStart && currentDateObj < sessionYearStart) {
        currentDateObj = new Date(sessionYearStart);
    } else if (sessionYearEnd && currentDateObj > sessionYearEnd) {
        currentDateObj = new Date(sessionYearEnd);
    }
    ...
    $hiddenDatePicker.datepicker({
        ...
        endDate: (function() {
            ...
            const s = parseISODate($('#sessionYearStart').val());
            const e = parseISODate($('#sessionYearEnd').val());
            if (s) s.setHours(0, 0, 0, 0);
            if (e) e.setHours(0, 0, 0, 0);

            // If today is within the session, limit to today
            if (s && e && now.getTime() >= s.getTime() && now.getTime() <= e.getTime()) {
                return now;
            }
            // Otherwise (future or past session), allow the whole session range
            return e || now;
        })(),
        rtl: isRTL()
    ...
    ============================================================ */
    if ($selectedDate.val()) {
        const initialDateStr = $selectedDate.val();
        let initialDateObj = new Date(initialDateStr);
        
        // Ensure initial date is within session range
        if (sessionYearStart && initialDateObj < sessionYearStart) {
            initialDateObj = new Date(sessionYearStart);
        } else if (sessionYearEnd && initialDateObj > sessionYearEnd) {
            initialDateObj = new Date(sessionYearEnd);
        }
        
        const finalDateStr = formatBackendDate(initialDateObj);
        const finalDisplayStr = formatDisplayDate(initialDateObj);
        
        if (finalDateStr !== initialDateStr) {
            $selectedDate.val(finalDateStr);
            $displayDate.val(finalDisplayStr);
            $dateDisplay.text(finalDisplayStr);
            $hiddenDatePicker.datepicker('setDate', initialDateObj);
        }
        
        refreshAttendanceData(finalDateStr);
    }
    ...
    prevBtn.addEventListener("click", () => {
        ...
        // Only allow if the resulting month-year is within sessionYearStart's month-year or later
        if (sessionYearStart) {
            const minMonth = new Date(sessionYearStart.getFullYear(), sessionYearStart.getMonth(), 1);
    ...
    nextBtn.addEventListener("click", () => {
        const nextMonth = new Date(currentDateObj);
        nextMonth.setMonth(nextMonth.getMonth() + 1);

        // Only allow if the resulting month-year is within sessionYearEnd's month-year or earlier
        if (sessionYearEnd) {
            const maxMonth = new Date(sessionYearEnd.getFullYear(), sessionYearEnd.getMonth(), 1);

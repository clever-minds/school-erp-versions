...
function packageTypeHistoryFormatter(value, row) {

    if (row.package_type == 1) {
        return "<span class='badge badge-primary'>" + window.trans["postpaid"] + "</span>";
    } else if (row.package_type == 0 || row.package_type != null) {
        return "<span class='badge badge-info'>" + window.trans["prepaid"] + "</span>";
    } else {
        return "<span>-</span>";
    }
...
function StudentNameFormatter(value, row) {
    let html = '';
    let url = baseUrl + '/reports/student/student-view-reports/' + row.user_id;
    if (row.role == 'Teacher') {
        url = baseUrl + '/reports/teacher/teacher-view-reports/' + row.user_id;
    } else if (row.role == 'Staff') {
        url = baseUrl + '/reports/staff/staff-view-reports/' + row.user_id;
    }
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.user.image) + ' <div class="ms-3"> <a href="' + url + '" class="text-dark"> <h6 class="mb-0">' + row.user.full_name + '</h6> </a> <small class="text-muted text-overflow-any"> ' + row.user.email + ' </small> </div> </div>';
    return html;
}

function staffLeaveNameFormatter(value, row) {
    let html = '';
    if (row.user.roles[0].name == 'Teacher') {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.user.image) + ' <div class="ms-3"> <a href="' + baseUrl + '/reports/teacher/teacher-view-reports/' + row.user_id + '" class="text-dark"> <h6 class="mb-0">' + row.user.full_name + '</h6> </a> <small class="text-muted text-overflow-any"> ' + row.user.email + ' </small> </div> </div>';
    } else {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.user.image) + ' <div class="ms-3"> <a href="' + baseUrl + '/reports/staff/staff-view-reports/' + row.user_id + '" class="text-dark"> <h6 class="mb-0">' + row.user.full_name + '</h6> </a> <small class="text-muted text-overflow-any"> ' + row.user.email + ' </small> </div> </div>';
    }
    return html;
}
...
function TeacherNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.image) + ' <div class="ms-3"> <a href="' + baseUrl + '/reports/teacher/teacher-view-reports/' + row.id + '" class="text-dark"> <h6 class="mb-0">' + row.full_name + '</h6> </a> <small class="text-muted text-overflow-any"> ' + row.email + ' </small> </div> </div>';
    return html;
}

function GuardianNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.email + ' </small> </div> </div>';
    return html;
}
...
function AssignmentSubmissionStudentNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.student.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.student.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.student.email + ' </small> </div> </div>';
    return html;
}
...
function taskTitleFormatter(value, row) {
    return '<div> <h6 class="mb-0">' + value + '</h6> <small class="text-muted">' + row.description + '</small> </div>';
}
...
function taskStatusFormatter(value, row) {
    ...
    // Parse due date correctly to avoid timezone issues
    let dueArr = row.plain_due_date.split('-');
    let dueDate = new Date(dueArr[0], dueArr[1] - 1, dueArr[2]);
    dueDate.setHours(0, 0, 0, 0);

    let isOverdue = (row.status == 'pending' || row.status == 'in_progress') && dueDate < today;

    let options = [
        { value: 'pending', label: window.trans['Pending'], class: 'badge-warning badge text-dark' },
        { value: 'in_progress', label: window.trans['In Progress'], class: 'badge-info badge text-white' },
        { value: 'completed', label: window.trans['Completed'], class: 'badge-success badge text-white' }
    ];

    if (isOverdue) {
        options.unshift({ value: 'overdue', label: window.trans['Overdue'], class: 'badge-danger badge text-white' });
    }

    let currentStatus = isOverdue ? 'overdue' : row.status;
    let currentOption = options.find(o => o.value == currentStatus) || options[0];
    let badgeClass = currentOption.class;

    let select = '<select class="form-control task-status-dropdown p-1 ' + badgeClass + '" data-id="' + row.id + '" style="width: 110px; height: auto; font-size: 11px; border: none; font-weight: 600; cursor: pointer; border-radius: 5px; text-align: center;">';
    options.forEach(option => {
        let selected = (currentStatus == option.value) ? 'selected' : '';
        select += '<option value="' + option.value + '" ' + selected + ' class="bg-white text-dark">' + option.label + '</option>';
    });
    select += '</select>';

    // Calculate status label text
    let statusLabel = '';
    ...
    if (row.status == 'completed') {
        if (dueDate < today) {
            statusLabel = '<div class="text-danger mt-1" style="font-size: 10px; font-weight: 600;">Overdue By ' + diffDays + daysText + '</div>';
        }
    } else {
        if (dueDate < today) {
            statusLabel = '<div class="text-danger mt-1" style="font-size: 10px; font-weight: 600;">Overdue By ' + diffDays + daysText + '</div>';
        } else if (dueDate > today) {
            statusLabel = '<div class="text-info mt-1" style="font-size: 10px; font-weight: 600;">Due In ' + diffDays + daysText + '</div>';
        } else {
            statusLabel = '<div class="text-primary mt-1" style="font-size: 10px; font-weight: 600;">Due Today</div>';
        }
    }

    return '<div class="d-flex flex-column align-items-center">' + select + statusLabel + '</div>';
}

function assigneeFormatter(value, row) {
    if (row.user) {
        return '<div class="d-flex align-items-center"> ' + imageFormatter(row.user.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.user.full_name + '</h6> </div> </div>';
    }
    return '-';
}
...
function taskActionFormatter(value, row) {
    return '<div class="d-flex justify-content-center"> ' +
        '<button type="button" class="btn btn-sm btn-icon btn-rounded btn-inverse-primary edit-data m-1" data-toggle="modal" data-target="#editModal" title="Edit"> <i class="fa fa-edit"></i> </button>' +
        '<button type="button" class="btn btn-sm btn-icon btn-rounded btn-inverse-dark delete-form m-1" data-url="' + baseUrl + '/tasks/' + row.id + '" title="Delete"> <i class="fa fa-trash"></i> </button>' +
        '</div>';
}
...
function StaffNameFormatter(value, row) {
    let html = '';
    if (row.school_id) {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.image) + ' <div class="ms-3"> <a href="' + baseUrl + '/reports/staff/staff-view-reports/' + row.id + '" class="text-dark"> <h6 class="mb-0">' + row.full_name + '</h6> </a> <small class="text-muted text-overflow-any"> ' + row.email + ' </small> </div> </div>';
    } else {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.email + ' </small> </div> </div>';
    }
...
function subjectNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.name + '</h6> <small class="text-muted text-overflow-any"> ' + row.code + ' </small> </div> </div>';
    return html;
}
...
function CreatedByNameFormatter(value, row) {
    if (row.created_by != null) {
        let html = '';
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.created_by.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.created_by.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.created_by.email + ' </small> </div> </div>';
        return html;
    }
}
function DriverNameFormatter(value, row) {
    let html = '';
    if (row.driver) {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.driver.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.driver.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.driver.email + ' </small> </div> </div>';
    } else {
        html = '-';
    }
    return html;
...
function HelperNameFormatter(value, row) {
    let html = '';
    if (row.helper) {
        html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.helper.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.helper.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.helper.email + ' </small> </div> </div>';
    } else {
        html = '-';
    }
    return html;
...
function SchoolNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.logo) + ' <div class="ms-3"> <h6 class="mb-0">' + row.name + '</h6> <small class="text-muted text-overflow-any"> ' + row.support_email + ' </small> </div> </div>';
    return html;
}

function SchoolNameSubscriptionFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.school.logo) + ' <div class="ms-3"> <h6 class="mb-0">' + row.school.name + '</h6> <small class="text-muted text-overflow-any"> ' + row.school.support_email + ' </small> </div> </div>';
    return html;
}
...
// Tracks guardian IDs that already have a file input rendered in this table load.
// Reset BEFORE each render cycle so every new table load starts with a clean slate.
window._renderedGuardianIds = window._renderedGuardianIds || new Set();

$(document).on('pre-body.bs.table', '#table_list', function () {
    window._renderedGuardianIds = new Set();
});

function guardianImageFormatter(value, row) {
    var guardianId = row.guardian.id;
    if (window._renderedGuardianIds.has(guardianId)) {
        // This guardian already has a file input in another row — do not render
        // another input to avoid sending an empty value that would override the upload.
        return '<span class="text-muted small"><i class="fa fa-link"></i> Shared guardian – upload from the first row</span>';
    }
    window._renderedGuardianIds.add(guardianId);

    return '<input type="file" name="guardian_image[' + guardianId + ']" accept="image/jpg,image/png,image/jpeg,image/svg">';
}
...
function schoolAdminFormatter(value, row) {
    ...
    html += '<p class="mt-1 text-facebook"><small class="text-overflow-any">' + row.user.email + '</small></p>';
    return html;
}

function linkFormatter(value, row) {
    if (row.link) {
        return "<a href='" + row.link + "' class='text-overflow-any' target='_blank'>" + row.link + "</a>";
    } else {
        return '-'
    }
}
...
function typeFormatter(value, row) {
    if (value == 1) {
        return '<div class="badge badge-primary badge-pill">' + window.trans['student'] + '</div>'
    } else if (value == 2) {
        return '<div class="badge badge-info badge-pill">' + window.trans['school_website'] + '</div>'
    } else if (value == 3) {
...
function FeesTransactionUserNameFormatter(value, row) {
    let html = '';
    html = '<div class="d-flex align-items-center"> ' + imageFormatter(row.user.image) + ' <div class="ms-3"> <h6 class="mb-0">' + row.user.full_name + '</h6> <small class="text-muted text-overflow-any"> ' + row.user.email + ' </small> </div> </div>';
    return html;
}
...
function feesTransactionParentGateway(value, row) {
    ...
    } else if (row.payment_gateway == 'Paystack' || row.payment_gateway == 'paystack') {
        return "<span class='badge badge-dark'>" + window.trans['Paystack'] + "</span>";
    } else {
        return "-";
    }
}
...
function timetableDayFormatter(value) {
    ...
    value.forEach(function (data) {
        html += "<li><b>" + data.title + " : </b><small>" + formatTimeAMPM(data.start_time) + " - " + formatTimeAMPM(data.end_time) + "</small></li>";
    })
    html += "</ol>";
    return html;
}
...
function teacherTimetableDayFormatter(value) {
    let html = "<ol>";
    value.forEach(function (data) {
        html += "<li><b>" + data.class_section.full_name + " - " + data.title + " : </b><small>" + formatTimeAMPM(data.start_time) + " - " + formatTimeAMPM(data.end_time) + "</small></li>";
    })
    html += "</ol>";
    return html;
}
...
function promoteStudentResultFormatter(value, row) {
    let result = (value !== null && value !== undefined && value !== "") ? value : 1; // Default to pass (1)
    let isPass = (result == 1) ? "checked" : "";
    let isFail = (result == 0) ? "checked" : "";

    return "<input type='hidden' name='promote_data[" + row.no + "][student_id]' value='" + row.user_id + "'>" +
        "<div class='d-flex'>" +
        "<div class='form-check mr-2'>" +
        "<label class='form-check-label'>" +
        "<input required type='radio' class='result form-check-input' name='promote_data[" + row.no + "][result]' value='1' " + isPass + ">" +
        window.trans["pass"] +
        " <i class='input-helper'></i></label>" +
        "</div>" +
        "<div class='form-check'>" +
        "<label class='form-check-label'>" +
        "<input type='radio' class='result form-check-input' name='promote_data[" + row.no + "][result]' value='0' " + isFail + ">" +
        window.trans["fail"] +
        " <i class='input-helper'></i></label>" +
        "</div>" +
        "</div>";
...
function promoteStudentStatusFormatter(value, row) {
    let status = (value !== null && value !== undefined && value !== "") ? value : 1; // Default to continue (1)
    let isContinue = (status == 1) ? "checked" : "";
    let isLeave = (status == 0) ? "checked" : "";

    return "<div class='d-flex'>" +
        "<div class='form-check form-check-info mr-2'>" +
        "<label class='form-check-label'>" +
        "<input required type='radio' class='status form-check-input' name='promote_data[" + row.no + "][status]' value='1' " + isContinue + ">" +
        window.trans["continue"] +
        "<i class='input-helper'></i></label>" +
        "</div>" +
        "<div class='form-check form-check-info'>" +
        "<label class='form-check-label'>" +
        "<input type='radio' class='status form-check-input' name='promote_data[" + row.no + "][status]' value='0' " + isLeave + ">" +
        window.trans["leave"] +
        " <i class='input-helper'></i></label>" +
        "</div>" +
        "</div>";
...
function promotionStatusFormatter(value, row) {
    if (value == 1) {
        return "<span class='badge badge-pill badge-success'>" + (window.trans["Promoted"] ?? "Promoted") + "</span>";
    }
    return "<span class='badge badge-pill badge-info'>" + (window.trans["Pending"] ?? "Pending") + "</span>";
}
...
function schoolActiveStatusFormatter(value, row) {
    ...
    // School is still being provisioned — show real-time mini progress bar
    const cfg = (window.schoolProvisionConfig && window.schoolProvisionConfig.installingSchools) || {};
    const state = cfg[row.id] || {};
    const pct = row.progress || state.progress || 0;
    const step = row.step_label || state.step_label || 'Starting Setup';

    return '<div class="school-installing-cell" id="provision-cell-' + row.id + '">'
        + '<span class="installing-label">INSTALLING...<span class="installing-pct"> ' + pct + '%</span></span>'
        + '<div class="mini-progress-bar"><div class="fill" style="width:' + pct + '%"></div></div>'
        + '<span class="installing-step">' + step + '</span>'
        + '</div>';
}
...
function totalFeesFormatter(value, row) {
    ...
    if (row.no.total_compulsory_fees_collected) {
        let html = '';
        html = row.no.total_compulsory_fees_collected ? row.no.currency_symbol + ' ' + amountFormatter(row.no.total_compulsory_fees_collected, null) : 0;

        if (row.no.compulsory_fees_sum_due_charges) {
            html += ' <div class="text-danger">(+ ' + row.no.currency_symbol + ' ' + amountFormatter(row.no.compulsory_fees_sum_due_charges, null) + ')</div>';
        }
        $('.total_compulsory_fees_collected').html(html);
    }
...
function classSectionFormatter(value, row) {
    ...
    if (row.is_linked === 1 || row.is_linked === '1') {
        ...
        html = value + '<span class="badge badge-info" style="font-size: 10px; padding: 2px 10px; border-radius: 999px; vertical-align: middle; margin-left: 5px;">' + (window.trans["Linked"] || "Linked") + '</span>';
    } else {
        html = value;
    }
    return html;
...
function applicationStatusFormatter(value, row) {
    ...
    } else if (row.application_status == 0) {
        return '<span class="badge badge-danger">' + window.trans['rejected'] + '</span>';
    } else {
        return '<span class="badge badge-warning">' + window.trans['pending'] + '</span>';
    }
...
function staffAttendanceUserFormatter(value, row) {
    ...
    return '<div class="d-flex align-items-center">' +
        ...
        '<h6 class="mb-0">' + row.user.full_name + '</h6>' +
        '</a>' +
        '</div>' +
        '</div>';
}
...
function examMarksStatusFormatter(value, row) {
    if (row.status == 1) {
        return '<span class="badge badge-success">' + window.trans['published'] + '</span>';
    } else if (row.status == 0) {
        return '<span class="badge badge-info">' + window.trans['draft'] + '</span>';
    } else {
        return '<span class="badge badge-warning">' + window.trans['pending'] + '</span>';
    }
}

function tripDelayFormatter(value, row) {
    if (!row.scheduled_time || !row.actual_time || row.actual_time === 'Pending') {
        return '<span class="text-muted">—</span>';
    }
    ...
    try {
        const schedMin = parseTime(row.scheduled_time);
        const actualMin = parseTime(row.actual_time);
        const diff = actualMin - schedMin;

        if (diff <= 0) {
            return '<span class="text-success font-weight-bold">0m</span>';
        } else {
            return '<span class="text-danger font-weight-bold">+' + diff + 'm</span>';
        }
    } catch (e) {
        return '<span class="text-muted">—</span>';
    }
...
function activeStatusLabelFormatter(value, row) {

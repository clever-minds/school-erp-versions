...
window.shiftEvents = {
    'click .edit-data': function (e, value, row) {
        ...
        let startTime = moment(row.start_time, 'HH:mm:ss').format('HH:mm');
        let endTime = moment(row.end_time, 'HH:mm:ss').format('HH:mm');
...
window.tableDescriptionEvents = {
    'click .bootstrap-table-description': function (e, value, row) {
        ...
        if (row.student) {
            $('.modal-title').html('<h3>Students</h3>');
            var listStudents = '';

...
window.expenseEvents = {
    'click .edit-data': function (e, value, row) {
        ...
        $('#edit_amount').val((row.amount).replaceAll(',', ''));
                ...
                $.each(row.extra_fields, function (index, value) {
                    ...
                    if (value.form_field.default_values && value.form_field.default_values.length) {
                        $.each(value.form_field.default_values, function (key) {
                            if (typeof (value.data) == 'object') {
                                ...
                                if (isJSON(value.data)) { // Checkbox
                                    let chkArray = [];
                                    chkArray = JSON.parse(value.data);
                        ...
                        if (value.form_field.type == 'file') {
                            if (value.data) {
                                $('#file_div_' + fieldName).removeClass('d-none').find('#file_link_' + fieldName).attr('href', value.data);
                            } else {
    ...
    'click .edit-data': function (e, value, row) {
        ...
        $.each(row.user.exam_marks, function (key, data) {
            ...
            let html_data = `<div class="row">` +
                            ...
                            `                           <input type="text" class="subject_name form-control" readonly name="edit[` + key + `][subject_name]" value="` + data.class_subject.subject.name_with_type + `" />` +
                            `                       </div>` +
                            `                       <div class="form-group col-sm-12 col-md-4">` +
...
window.feesClassEvents = {
    'click .edit-data': function (e, value, row) {
        $('#edit_class_id').val(row.class_id).trigger('change').attr('disabled', true)
        $('#class_id').val(row.class_id)
        if (row.fees_class) {
            var feesClassData = [];
            $.each(row.fees_class, function (index, value) {
                feesClassData.push({
                    id: value.id,
                    fees_type_id: value.fees_type_id,
                    amount: value.amount,
                    choiceable: (value.choiceable == 1 ? '1' : '0')
                });
            });
            addNewClassFeesData.setList(feesClassData);

            // $.each(row.fees_class, function (index, value) {
            //     $('#remove-fees-data-' + index).attr('data-id', value.id);
            // });
        }
    }
};
window.feesPaidEvents = {
    'click .compulsory-data': function (e, value, row, index) {
        ...
        $('#compulsory-fees-id').val(row.fees_data.id);
        $('#student-id').val(row.student_id);
        $('#class-id').val(row.class_id);

        $('.student-name').html(`${row.student_name} :- ${row.class_name}`);
        $('.paid-date').val(row.current_date);
        $document.find('.cheque_no').val(null);

        if (row.fees_data.compulsory_fees.length) {
            $document.find('.mode-container').show(200);
            $('.compulsory-div').show();
            const feesTotalAmount = (row.fees_data.due_charges_applicable ? row.fees_data.compulsory_with_due_charges.toFixed(2) : row.fees_data.total_compulsory_amount.toFixed(2));

            const html = generateCompulsoryFeesTable(row.fees_data, row.fees_status);
    ...
    'click .optional-data': function (e, value, row, index) {
        ...
        // Add data in Modal
        $('#optional_fees_id').val(row.fees_data.id);
        ...
        if (row.fees_data.optional_fees.length) {
            $optionalDiv.show();

            // Declare HTML using a template
            let html = '<table class="table"><tbody>';
            row.fees_data.optional_fees.forEach((value, index) => {
                html += '<tr>';
                if (value.is_paid) {
                    if (value.date) {
...
window.languageSettingsEvents = {
    'click .edit-data': function (e, value, row) {
        ...
        $('#code').val(row.code);
        if (row.is_rtl) {
            $('#rtl').prop('checked', true); // set CheckBox True
        } else {
            $('#rtl').prop('checked', false); // set CheckBox False
        }
    }
};
...
window.feesEvents = {
    'click .edit-data': function (e, value, row) {
        ...
        if (row.include_fee_installments == 1) {
            $('.edit-fees-installment-content').show(200)
        } else {
            $('.edit-fees-installment-content').hide(200)
        }
...
window.subscriptionEvents = {
    'click .edit-data': function (e, value, row) {
        ...
        var start_date = new Date(row.subscription.start_date);
        var start_date = new Intl.DateTimeFormat('en-US', options).format(start_date);

        var end_date = new Date(row.subscription.end_date);
        var end_date = new Intl.DateTimeFormat('en-US', options).format(end_date);
        $('.billing_cycle').html(start_date + ' To ' + end_date);

        $('.total-student').html(row.total_student);
        $('.student-charge').html((row.subscription.student_charge).toFixed(4));
        $('.total-student-charge').html((row.total_student * row.subscription.student_charge).toFixed(4));

        $('.total-staff').html(row.total_staff);
        $('.staff-charge').html((row.subscription.staff_charge).toFixed(4));
        $('.total-staff-charge').html((row.total_staff * row.subscription.staff_charge).toFixed(4));

        var total_user_charges = (row.total_student * row.subscription.student_charge) + (row.total_staff * row.subscription.staff_charge);
        ...
        $.each(row.addons, function (index, value) {
            ...
            html += '<td class="text-right">' + (value.price).toFixed(4) + '</td>';
            html += '</tr>';
            total_addon += value.price;
        });
        html += '<tr>';
        html += '<th colspan="4">' + window.trans['total_addon_charges'] + ' :</th>';
        html += '<th class="text-right">' + row.currency_symbol + ' ' + total_addon.toFixed(4) + '</th>';
        html += '</tr>';
        ...
        html += '<th class="text-right">' + row.currency_symbol + ' ' + (total_addon + total_user_charges).toFixed(2) + '</th>';
        html += '</tr>';

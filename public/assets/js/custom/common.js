...
$('.create-form-without-reset-text-editor').on('submit', function (e) {
    ...
    setTimeout(() => {
        ...
        var data = request_data.get('data');
        let formElement = $(this);
        let submitButtonElement = $(this).find(':submit');
        let url = $(this).attr('action');   
        ...
        formAjaxRequest('POST', url, request_data, formElement, submitButtonElement, successCallback);
    }, 1000);
...
$(document).on('click', '.select-plan', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            function successCallback(response) {
                if (response.data) {
                    confirm_upcomcing_plan(response.data.package_id);
                } else {
                    showSuccessToast(response.message);
                    setTimeout(() => {
                        location.reload();
...
function confirm_upcomcing_plan(package_id) {
    let id = package_id;
    Swal.fire({
        title: window.trans["Are you sure"],
        text: window.trans["Accept Upcoming Billing Cycle Subscription"],
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function already_added_upcoming_plan() {
    Swal.fire({
        text: window.trans["You have already selected an upcoming billing cycle plan If you want to change your upcoming billing cycle plan please ensure to remove the previously selected plan before making any alterations"],
        icon: 'error',
    })
}
...
$(document).on('click', '.add-addon', function (e) {
    ...
    Swal.fire({
        title: window.trans["Are you sure"],
        text: window.trans["Agree to This Addon"],
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
$(document).on('click', '.no-feature-lock-menu,.no-feature-lock-menu-sub-item', function (e) {
    ...
    if (role == 'School Admin') {
        Swal.fire({
            ...
            confirmButtonColor: '#3085d6',
            confirmButtonText: window.trans["ok"], html: '<div class="mb-2">' + window.trans["no_permission_upgrade_plan"] + '</div><a href="' + link + '">' + window.trans["click_here_to_buy_addon"] + '</a>',
        })

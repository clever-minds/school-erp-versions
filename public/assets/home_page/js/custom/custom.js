...
$('#subject-id').on('change', function () {
    ...
    // Check if any class sections are selected
    if ($('#class-section-id').val().length == 0) {
        $('#topic-lesson-id').val("data-not-found").attr('disabled', true).show();
        return;
    }

    if (selectedSubjectId === "data-not-found") {
        $('#topic-lesson-id').val("data-not-found").attr('disabled', true).show();
        return;
    }
    ...
    let shownLessonIds = new Set(); // To track unique lesson IDs
    ...
    $('#class-section-id').val().forEach(function (classSectionId) {
        $('#topic-lesson-id option').each(function () {
            ...
            if ($(this).data('subject') == selectedSubjectId && !shownLessonIds.has(lessonId)) {
                ...
                shownLessonIds.add(lessonId); // Add the lesson ID to the set
                lessonOptionsFound = true; // Mark that we found at least one lesson option
            }
        });
    });

    // If no lesson options were found, disable the dropdown and show a message
    ...
    function renderAttendanceTable(data, month, year) {
        ...
        attendanceList.forEach(entry => {
            ...
            if (!groupedByUser[userId]) {
                groupedByUser[userId] = { user: entry.user, records: [] };
            }
            groupedByUser[userId].records.push(entry);
        });

        Object.values(groupedByUser).forEach(({ user, records }) => {
            ...
            userCell.innerHTML = `
                <div class="d-flex align-items-center">
                    <a data-toggle="lightbox" href="${imageSrc}">
                        <img src="${imageSrc}" class="rounded-circle border"
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const attendance = records.find(r => r.date === dateStr);

                if (holiday) {
                    cell.innerHTML = `<i class="fa fa-circle text-info" title="${holiday.title || 'Holiday'}"></i>`;
                } else if (attendance) {
                    switch (attendance.status) {
                        case "present": cell.innerHTML = `<i class="fa fa-circle text-success"></i>`; break;
                        case "absent": cell.innerHTML = `<i class="fa fa-circle text-danger"></i>`; break;
                        default: cell.innerHTML = `<i class="fa fa-circle text-secondary"></i>`;
                    }
                } else {

    ...
    <script>
        ...
        $(document).ready(function () {
            $('body').on('focus', ".leave-date", function () {
                let today = moment();
                let sessionStart = moment("{{ $current_session_year->original_start_date }}", "YYYY-MM-DD");
                let sessionEnd = moment("{{ $current_session_year->original_end_date }}", "YYYY-MM-DD");

                // minDate is the later of today OR the first day of the session
                // This prevents backdated requests while allowing future session requests if viewing upcoming session
                let minDate = moment.max(today, sessionStart).format('DD-MM-YYYY');
                let maxDate = sessionEnd.format('DD-MM-YYYY');

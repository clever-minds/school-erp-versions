...
<div class="border border-secondary rounded-lg my-4 mx-1">
    ...
    <div class="row my-4 mx-1">
        <div class="form-group col-md-4 col-sm-12">
            ...
            <input name="system_name" id="system_name" value="{{ $settings['system_name'] ?? '' }}" type="text"
                required placeholder="{{ __('system_name') }}" class="form-control" />
        </div>

        <div class="form-group col-md-4 col-sm-12">
            <label for="mobile">{{ __('mobile') }} <span class="text-danger">*</span></label>
            <input name="mobile" id="mobile" value="{{ $settings['mobile'] ?? '' }}" type="number" required
        ...
        <div class="form-group col-md-6 col-lg-6 col-xl-4 col-sm-12">
            ...
            <select name="date_format" id="date_format" required class="form-control">
                @foreach ($getDateFormat as $key => $dateformat)
                    <option value="{{ $key }}"
                        {{ isset($settings['date_format']) && $settings['date_format'] == $key ? 'selected' : '' }}>
                        {{ $dateformat }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-md-6 col-lg-6 col-xl-4 col-sm-12">
            ...
            <select name="time_format" id="time_format" required class="form-control">
                @foreach ($getTimeFormat as $key => $timeFormat)
                    <option value="{{ $key }}"
                        {{ isset($settings['time_format']) && $settings['time_format'] == $key ? 'selected' : '' }}>
                        {{ $timeFormat }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="row my-4 mx-1">
        ...
        <div class="form-group col-md-6 col-lg-6 col-xl-4 col-sm-12">
            <label for="login_page_logo">{{ __('login_page_image') }} <span class="text-danger">*</span> <small
                    class="text-muted">(Recommended size: 920x1080 pixels)</small></label>
        ...
        <div class="form-group col-md-6 col-lg-6 col-xl-4 col-sm-12">
            <label for="student_web_background_image">{{ __('student_web_background_image') }}<small
                    class="text-muted">(Recommended size: 1920x1080
                    pixels)</small></label>
            ...
            <div class="input-group col-xs-12">
                <input type="text" class="form-control file-upload-info" id="student_web_background_image"
                    disabled="" placeholder="{{ __('student_web_background_image') }}" />
                <span class="input-group-append">
                    <button class="file-upload-browse btn btn-theme" type="button">{{ __('upload') }}</button>
                </span>
                <div class="col-md-12 mt-2">
                    <img height="100px" width="100px"
                        src='{{ !empty($settings['student_web_background_image']) ? url($settings['student_web_background_image']) : asset('assets/no_image_available.jpg') }}'
        ...
        <div class="form-group col-md-6 col-lg-6 col-xl-4 col-sm-12">
            ...
            <input name="theme_color" id="theme_color" value="{{ $settings['theme_color'] ?? '' }}" type="text"
        ...
        <div class="form-group col-md-6 col-sm-12">
            ...
            <input type="number" min="1" max="{{ str_replace('M', '', ini_get('upload_max_filesize')) }}"
                class="form-control" value="{{ $settings['file_upload_size_limit'] ?? 0 }}"
                id="file_upload_size_limit">
        ...
        <div class="form-group col-md-4 col-sm-12">
            ...
            <div class="d-flex">
                <div class="form-check form-check-inline">
                    <label class="form-check-label">
                        {!! Form::radio('school_inquiry', '1', false, [
                            'class' => 'default',
                            ($settings['school_inquiry'] ?? 0) == 1 ? 'checked' : '',
                        ]) !!}{{ __('enable') }}
                    </label>
                </div>
                <div class="form-check form-check-inline">
                    <label class="form-check-label">
                        {!! Form::radio('school_inquiry', '0', false, [
                            'class' => 'custom',
                            ($settings['school_inquiry'] ?? 0) == 0 ? 'checked' : '',
                        ]) !!}{{ __('disable') }}
                    </label>
                </div>
            </div>
        ...
        <div class="form-group col-md-6 col-sm-12">
            ...
            <input name="student_web_url" id="student_web_url" value="{{ $settings['student_web_url'] ?? '' }}"
                type="text" placeholder="{{ __('student_web_url') }}" class="form-control" />
        </div>
    </div>
</div>
{{-- End System Settings --}}

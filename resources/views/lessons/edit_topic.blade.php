                                    ...
                                    <select name="class_section_id[]" id="class-section-id" class="class_section_id form-control select2-dropdown select2-hidden-accessible" multiple>
                                        @foreach ($class_section as $section)
                                            <option value="{{ $section->id }}" 
                                                    ...
                                                    @if($topic->lesson_topic_class->pluck('class_section_id')->contains($section->id)) selected @endif>
                                                {{ $section->full_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-sm-12 col-md-4">
                                    ...
                                    <select name="class_subject_id" id="class-subject-id" class="form-control">
                                        ...
                                        @foreach ($subjectTeachers as $item)
                                            <option value="{{ $item->subject_id }}" data-class-id="{{ $item->class_section_id }}" data-user="{{ Auth::user()->id }}">{{ $item->subject_with_name}}</option>
                                        @endforeach
                                    </select>
    ...
    <script>
        $(document).ready(function () {
            ...
            @if($topic->lesson && $topic->lesson->lesson_commons)
                ...
                var classSectionIds = @json($topic->lesson_topic_class->pluck('class_section_id'));
                ...
                @if($topic->lesson->lesson_commons->first() && $topic->lesson->lesson_commons->first()->syllabus->class_subject)
                    $('#class-subject-id')
                        .val({{ $topic->lesson->lesson_commons->first()->syllabus->subject_id }})
                        .trigger('change')
                        .attr('disabled', true);

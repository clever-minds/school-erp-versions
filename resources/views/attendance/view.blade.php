...
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="show_attendance_student_list">
                            <table aria-describedby="mydesc" class='table student_table' id='table_list' data-toggle="table"
                                data-url="{{ route('attendance.list.show', 1) }}" data-click-to-select="true"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" data-field="student_id" data-sortable="true" data-visible="false">
                                            {{__('user_id')}}
                                        </th>
                                        <th scope="col" data-field="user.student.id" data-sortable="true"
                                            data-visible="false">{{__('student_id')}}</th>
                                        <th scope="col" data-field="user.student.admission_no" data-sortable="true">
                                            {{__('admission_no')}}
                                        </th>
                                        <th scope="col" data-field="user.student.roll_number" data-sortable="true">
                                            {{__('roll_no')}}
                                        </th>
                                        <th scope="col" data-field="user.full_name">{{__('name')}}</th>
                                        <th scope="col" data-field="type" data-formatter="attendanceTypeFormatter"
                                            data-escape="false">{{__('type')}}</th>
    ...
    {{-- @dd($sessionYear->getRawOriginal('start_date')) --}}
@endsection
...
@section('script')
    <script>
        ...
        $('body').on('focus', ".datepicker-popup", function () {
            ...
            let startdate = formatDate("{{ $sessionYear->getRawOriginal('start_date') }}");
            let endDate = formatDate("{{ $sessionYear->getRawOriginal('end_date') }}");
            ...
            let highlightToday = today >= start && today <= end;

            let finalEndDate = endDate;
            if (today >= start && today <= end) {
                finalEndDate = "{{ Carbon\Carbon::now()->format('d-m-Y') }}";
            }

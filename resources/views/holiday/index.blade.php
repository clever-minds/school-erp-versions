                            ...
                            <form class="create-form pt-3" id="create-form" action="{{route('holiday.store')}}" method="POST"
                                ...
                                <div class="row">
                                    <div class="form-group col-sm-12 col-md-6">
                                        ...
                                        {!! Form::text('date', null, ['required', 'placeholder' => __('date'), 'class' => 'datepicker-popup form-control', 'autocomplete' => 'off', 'id' => 'create-date']) !!}
                                        <span class="input-group-addon input-group-append">
                                        </span>
                                        <div id="create-leave-msg" class="text-danger mt-2"></div>
                                    </div>
                                    <div class="form-group col-sm-12 col-md-6">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row form-group">
                            <div class="col-sm-12 col-md-12">
                                ...
                                <div id="edit-leave-msg" class="text-danger mt-2"></div>
                            </div>
                        </div>
                        <div class="row form-group">
                            <div class="col-sm-12 col-md-12">
...
@section('js')
    <script>
        let schoolDateFormat = "{{$schoolSettings['date_format'] ?? 'Y-m-d'}}";
        $(function () {
            const sessionStart = new Date("{{ \Carbon\Carbon::createFromFormat(($schoolSettings['date_format'] ?? 'Y-m-d'), $current_sessionYear->start_date)->format('Y-m-d') }}");
            const sessionEnd = new Date("{{ \Carbon\Carbon::createFromFormat(($schoolSettings['date_format'] ?? 'Y-m-d'), $current_sessionYear->end_date)->format('Y-m-d') }}");
            ...
            $('.datepicker-popup').datepicker({
                ...
                var date = $(this).val();
                var formType = $(this).attr('id') === 'create-date' ? 'create' : 'edit';
                checkApprovedLeaves(date, formType);
            });

            function checkApprovedLeaves(date, formType) {
                if (date) {
                    $.ajax({
                        url: "{{ route('holiday.check-approved-leaves') }}",
                        type: "GET",
                        data: {
                            date: date
                        },
                        success: function (response) {
                            var msgDiv = formType === 'create' ? $('#create-leave-msg') : $('#edit-leave-msg');
                            if (response.count > 0) {
                        ...
                        error: function () {
                            console.error("Error fetching approved leaves count");
                        }
                    });
                }
            }

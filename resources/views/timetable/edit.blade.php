            ...
            <div class="col-md-12 grid-margin stretch-card search-container">
                <div class="card">
                    <div class="card-body">
                        <div class="row mb-3">
                            ...
                            <div class="text-center col-md-10 col-sm-12 col-12">
                                ...
                                @if ($classSection->class->shift)
                                    <h6 class="text-info italic">({{ __('Note') }}: {{ __('Timetable timings are based on') }} {{ $classSection->class->shift->name }} {{ __('shift') }} {{ \Carbon\Carbon::parse($timetableSettingsData['timetable_start_time'])->format('h:i A') }} - {{ \Carbon\Carbon::parse($timetableSettingsData['timetable_end_time'])->format('h:i A') }})</h6>
                                @endif
                        ...
                        <div class="row">
                            <div id='external-events' class="col-md-2 col-sm-12 col-12" style="max-height: 1200px; overflow-y: auto; overflow-x: hidden;">
                                ...
                                @foreach ($subjectTeachers as $subjectTeacher)
                                    ...
                                    <div class='fc-event fc-h-event fc-div-color fc-daygrid-event fc-daygrid-block-event text-wrap'
                                        ...
                                        <div class='fc-event-main px-2' style="width: -webkit-fill-available"
                                            data-subject-type="{{ $subjectTeacher->class_subject ? $subjectTeacher->class_subject->type : '' }}">
                                            {{-- {{ Str::limit($subjectTeacher->subject->name . ' ( ' .
                                            $subjectTeacher->subject->type . ' ) ' . ' - ' .
                                            $subjectTeacher->teacher->full_name, 25, ' ...') }} --}}
                                ...
                                @foreach ($subjectWithoutTeacherAssigned as $subject)
                                    ...
                                    <div class='fc-event fc-h-event fc-div-color fc-daygrid-event fc-daygrid-block-event'
                                        ...
                                        <div class='fc-event-main' data-subject-type="{{ $filtered['type'] ?? '' }}">
                                            {{  Str::limit($subject->name . ' ( ' . $subject->type . ' )', 25, ' ...') }}
                                        </div>
                                    </div>
                                @endforeach
...
@section('js')
    <script>
        ...
        $(document).ready(function () {
            ...
            // Override height to enable internal scrolling
            createTimetable.setOption("height", 800);
            createTimetable.setOption("contentHeight", "auto");
        })

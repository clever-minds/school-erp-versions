...
@section('css')
    <link rel="stylesheet" href="{{ asset('assets/css/school-provision.css') }}">
@endsection
...
@section('content')
    <div class="content-wrapper">
        ...
        @if(count($installingSchools) > 0)
            <div id="provision-banner" class="col-lg-12 mb-2">
                <div class="banner-icon"><i class="fa fa-cog fa-spin"></i></div>
                <div class="banner-pulse"></div>
                <div>
                    <strong>{{ __('active_provisioning') }}</strong><br>
                    <span id="provision-banner-count">{{ __('currently_setting_up') }} {{ count($installingSchools) }} {{ __('school_instance')}}{{ count($installingSchools) > 1 ? 's' : '' }}.</span>
                </div>
            </div>
        @else
            <div id="provision-banner" class="col-lg-12 mb-2 d-none">
                <div class="banner-icon"><i class="fa fa-cog fa-spin"></i></div>
                <div class="banner-pulse"></div>
                <div>
                    <strong>{{ __('active_provisioning') }}</strong><br>
                    <span id="provision-banner-count"></span>
                </div>
            </div>
        @endif
                    ...
                    <div class="card-body">
                        <form class="create-form school-registration-form school-registration-validate" enctype="multipart/form-data" action="{{ route('schools.store') }}" method="POST" novalidate="novalidate">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="school_support_email">{{ __('school_email') }} <span class="text-danger">*</span></label>
                                    <input type="email" name="school_support_email" id="school_support_email" placeholder="{{__('support_email')}}" class="form-control" required>
                                </div>
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="school_support_phone">{{ __('school_phone') }} <span class="text-danger">*</span></label>
                                    <input type="number" name="school_support_phone" maxlength="16" id="school_support_phone" pattern="[0-9]{6,15}" 
                                            title="Please enter a valid mobile number (6-15 digits)" placeholder="{{__('support_phone')}}" min="0" class="form-control remove-number-increment" required>
                                </div>
                                <div class="form-group col-sm-12 col-md-6">
                                ...
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="">{{ __('board')}} </label>
                                    {!! Form::select('board_id', $boards, null, ['class' => 'form-control', 'placeholder' => __('select_board')]) !!}
                                </div>

                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="school_domain">{{ __('School Code Prefix')}} <span class="text-danger">*</span></label>
                                    <div class="input-group mb-3">
                                        <input type="text" class="form-control school_code_prefix form-control-right-radius-0" id="school_code_prefix" name="school_code_prefix" required placeholder="{{ __('prefix') }}" value="{{ $prefix }}">
                                        <div class="input-group-append">
                                            <input type="text" class="input-group-text form-control text-body school_code form-control-left-radius-0" id="basic-addon2" name="school_code" value="{{ $school_code }}" readonly>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">    
                                <div class="form-group col-sm-12 col-md-4 defaultDomain" style="display: none">
                                    ...
                                    <div class="input-group mb-3">
                                            <input type="text" class="form-control form-control-right-radius-0 domain-pattern" name="domain" placeholder="{{ __('domain') }}" aria-label="Recipient's username" aria-describedby="basic-addon2" disabled>
                                        <div class="input-group-append">
                                            <span class="input-group-text form-control text-body form-control-left-radius-0" id="basic-addon2">.{{ $baseUrlWithoutScheme }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group col-sm-12 col-md-4 customDomain" style="display: none">
                                    ...
                                    <div class="alert alert-info" role="alert">
                                            {!! __('note_add_custom_domain_without_https_and_www') !!}
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class='form-group col-xl-4 col-lg-6 col-md-6 col-sm-12'>
                                            ...
                                            @elseif($data->type == 'number')
                                                ...
                                                @elseif($data->type == 'dropdown')
                                                    {{ Form::select(
                                                        'extra_fields['.$key.'][data]',array_combine($data->default_values, $data->default_values),
                                                        null,
                                                        [
                                                            'id' => $fieldName,
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row" id="toolbar">
                            ...
                            <div class="form-group col-sm-12 col-md-4">
                                <label class="filter-menu" for="package">{{ __('board') }}</label>
                        ...
                        <table aria-describedby="mydesc" class='table' id='table_list'
                            ...
                            <tr>
                                ...
                                <th scope="col" data-field="support_phone">{{__('school_phone')}}</th>
                                ...
                                <th scope="col" data-field="admin_id" data-visible="false">{{ __('admin_id')}}</th>
                                <th scope="col" data-field="user" data-formatter="schoolAdminFormatter">{{ __('school_admin') }}</th>
                ...
                <form id="edit-form" class="pt-3 edit-form" action="{{ url('schools') }}">
                    ...
                    <div class="modal-body">
                        <div class="row">                        
                            ...
                            <div class="form-group col-sm-12 col-md-3">
                                <label for="edit_school_support_email">{{ __('school_email') }} <span class="text-danger">*</span></label>
                                <input type="email" name="edit_school_support_email" id="edit_school_support_email" placeholder="{{__('support_email')}}" class="form-control" required>
                            </div>
                            <div class="form-group col-sm-12 col-md-3">
                                <label for="edit_school_support_phone">{{ __('school_phone') }} <span class="text-danger">*</span></label>
                                <input type="number" name="edit_school_support_phone" min="0" id="edit_school_support_phone" placeholder="{{__('support_phone')}}" class="form-control remove-number-increment" required>
                            </div>

                            <div class="form-group col-sm-12 col-md-3">
                                <label for="edit_school_board_id">{{ __('board')}} </label>
                                {{ Form::select('board_id', $boards, null, ['class' => 'form-control', 'id' => 'edit_board_id', 'placeholder' => __('select_board')]) }}
                            </div>
                        ...
                        <div class="row">    
                            <div class="form-group col-sm-12 col-md-4 defaultDomain" style="display: none">
                                <label for="school_domain">{{ __('default_domain')}}</label>
                                <div class="input-group mb-3">
                                        <input type="text" class="form-control domain-pattern form-control-right-radius-0" id="edit_default_domain" name="edit_domain" placeholder="{{ __('domain') }}" aria-label="Recipient's username" aria-describedby="basic-addon2" disabled>
                                    <div class="input-group-append">
                                        <span class="input-group-text text-body form-control form-control-left-radius-0" id="basic-addon2">.{{ $baseUrlWithoutScheme }}</span>
                                    </div>
                                </div>
                                <div class="mt-2">
                            ...
                            <div class="row other-details">
                                ...
                                @foreach ($extraFields as $key => $data)
                                    ...
                                    <div class='form-group col-md-12 col-lg-6 col-xl-4 col-sm-12'>
                                        ...
                                        @elseif($data->type == 'dropdown')
                                            {{ Form::select(
                                                'edit_extra_fields['.$key.'][data]',array_combine($data->default_values, $data->default_values),
                                                null,
                                                [
                                                    'id' => 'edit_'.$fieldName,
                                                    'class' => 'form-control select-fields',
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-sm-12 col-md-12">
                                ...
                                <input type="hidden" id="admin_modal_school_name" name="school_name">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-sm-12 col-md-4 demo-school">
                                <div class="d-flex">
                                    <div class="form-check w-fit-content">
                                        <label class="form-check-label ml-4">
    ...
    <div class="modal fade" id="installProgressModal" tabindex="-1" role="dialog" aria-labelledby="installProgressModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div>
                        <div class="modal-title" id="installProgressModalLabel">{{ __('installation_progress') }}</div>
                        <div class="school-name-sub" id="ipm-school-name">—</div>
                    </div>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true"><i class="fa fa-times" style="color:#fff"></i></span>
                    </button>
                </div>
                <div class="modal-body p-0">

                    {{-- Circular Progress Ring --}}
                    <div class="ipm-ring-wrap">
                        <div class="ipm-ring">
                            <svg viewBox="0 0 110 110">
                                ...
                                <circle class="ring-fill" id="ipm-ring-fill" cx="55" cy="55" r="45"
                                    stroke-dasharray="282.7"
                                    stroke-dashoffset="282.7"/>
                            </svg>
                            <div class="ipm-ring-value" id="ipm-ring-value">0%</div>
                        </div>
                        <div class="ipm-step-label" id="ipm-step-label">{{ __('initializing') }}...</div>
                        <div class="ipm-step-sub">{{ __('wait_while_we_provision_the_environment') }}...</div>
                    </div>

                    {{-- Step Checklist --}}
                    <ul class="ipm-steps">
                        <li id="ipm-step-job_started" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('starting_setup') }}
                        </li>
                        <li id="ipm-step-database_created" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('database_creation') }}
                        </li>
                        <li id="ipm-step-migrations_completed" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('schema_migration') }}
                        </li>
                        <li id="ipm-step-seeders_executed" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('configuration_sync') }}
                        </li>
                        <li id="ipm-step-school_activated" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('administrative_setup') }}
                        </li>
                        <li id="ipm-step-welcome_email_sent" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('welcome_email') }}
                        </li>
                        <li id="ipm-step-verification_email_sent" class="pending">
                            <div class="step-icon pending"><i class="fa fa-circle-o"></i></div>
                            {{ __('verification_email') }}
                        </li>
                    </ul>
                </div>
                ...
                <div class="ipm-footer">
                    <button type="button" class="btn-minimize-tracking" data-dismiss="modal">
                        {{ __('minimize_tracking') }}
                    </button>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // View progress modal trigger — delegated because rows are rendered dynamically by bootstrap-table
        $(document).on('click', '.view-provision-progress', function (e) {
            e.preventDefault();
            var schoolId   = parseInt($(this).data('school-id'));
            var schoolName = $(this).data('school-name') || 'School';

            // Ensure SchoolProvision is loaded and the school state is registered
            if (window.SchoolProvision) {
                // Register school name if not already in state
                if (window.schoolProvisionConfig && !window.schoolProvisionConfig.installingSchools[schoolId]) {
                    window.schoolProvisionConfig.installingSchools[schoolId] = {
                        name: schoolName, progress: 5, step_label: 'Starting Setup', status: 'installing'
                    };
                }
                if (window.schoolProvisionConfig && window.schoolProvisionConfig.installingSchools[schoolId]) {
                    window.schoolProvisionConfig.installingSchools[schoolId].name = schoolName;
                }
                window.SchoolProvision.openModal(schoolId);
            }
        });
    });
...
{{-- Pusher SDK (required for Reverb) --}}
<script src="https://js.pusher.com/8.4.0/pusher.min.js"></script>

{{-- Inject school provisioning config before the module loads --}}
<script>
    window.schoolProvisionConfig = {
        reverb: @json($reverb),
        installingSchools: @json($installingSchools),
    };
</script>

<script src="{{ asset('assets/js/school-provision.js') }}"></script>

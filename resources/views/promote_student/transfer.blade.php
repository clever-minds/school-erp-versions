                ...
                <div class="col-md-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            {{-- Add note only default session year  --}}
                            <div class="alert alert-info">
                                <strong>{{ __('note') }} :</strong> {{ __('Only default session year students will be transferred') }} ({{ $defaultSessionYear->name }})
                            </div>
                            ...
                            <form action="{{ route('transfer-student.store') }}" data-success-function="formSuccessFunction" class="create-form mt-6 pt-3" id="formdata">
                                ...
                                <div class="row" id="toolbar1">
                                    {{-- <div class="form-group col-sm-12 col-md-3">
                                        ...
                                        <select required name="transfer_session_year_id" id="transfer_session_year_id" class="form-control select2" style="width:100%;" tabindex="-1" aria-hidden="true">
...
@section('script')
    <script>
        ...
        $('#student_class_section, #from_session_year_id').on('change', function () {
            $('#promote_student_table_list').bootstrapTable('refresh');
        });

        $('#transfer_session_year_id').on('change', function () {
            $('#transfer-student-table-list').bootstrapTable('refresh');
        });
        ...
        $('#from_session_year_id').on('change', function () {
            ...
            // Show and enable all options in Promote In dropdown first
            ...
            if (selectedStartDate) {
                ...
                // Find the next session chronological session
                ...
                if (nextSessionId) {
                    ...
                    // Hide and disable all options except the next one and the empty placeholder
                    $('#session_year_id option').each(function () {
                ...
                // If Promote From is cleared, clear Promote In as well
                $('#session_year_id').val("").trigger('change');
                $('#session_year_id_hidden').val("");
            }
        });

        // Remove the old Select2 readonly hack as it's no longer needed with the disabled + hidden input approach
        ...
        function set_data() {
            $(document).ready(function () {
                ...
                if (student_class != '' && session_year != '' && promote_class != '' && from_session_year != '' && selectedRows.length > 0) {
                    $('.btn_promote').show();
                } else {
                    $('.btn_promote').hide();
                }
        ...
        $('#student_class_section,#session_year_id, #session_year_id, #new_student_class_section').on('change', function () {
            set_data();
        });
        ...
        $('.btn_promote').on('click', function (e) {
            ...
            // Disable inputs for unchecked rows so they don't get submitted
            ...
            // Re-enable inputs after a short delay to keep UI consistent
            setTimeout(() => {

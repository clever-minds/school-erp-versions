...
<div class="card shadow-sm border-0 tvr-card mb-4">
    <div class="card-body p-4">
        ...
        <div class="row mb-4">
            <div class="col-md-6 mb-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <span class="font-weight-bold text-dark">{{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($salary_structure['basic_salary'], 2) }}</span>
                </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <span class="font-weight-bold text-dark">{{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($salary_structure['total_allowance'], 2) }}</span>
                </div>
                <hr class="my-2 tvr-divider">
            </div>
            <div class="col-md-6 mb-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <span class="font-weight-bold text-dark">{{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($salary_structure['total_deduction'], 2) }}</span>
                </div>
                <hr class="my-2 tvr-divider">
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center rounded p-3 mb-2" style="background-color: #e8fdf0;">
            ...
            <span class="font-weight-bold" style="color: #10b981; font-size: 18px;">{{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($salary_structure['net_salary'], 2) }}</span>
        </div>
    </div>
</div>
                            ...
                            <tbody>
                                @foreach($salary_structure['allowances'] as $item)
                                    <tr>
                                        ...
                                        <td class="text-right font-weight-bold text-success">+ {{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($item['amount'], 2) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <table class="table mb-0 tvr-table">
                            ...
                            <tbody>
                                @foreach($salary_structure['deductions'] as $item)
                                    <tr>
                                        <td class="text-dark font-weight-medium">{{ $item['name'] }}</td>
                                        <td class="text-right font-weight-bold text-danger">- {{ $schoolSettings['currency_symbol'] ?? '$' }} {{ number_format($item['amount'], 2) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>

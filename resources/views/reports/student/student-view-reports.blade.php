            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a
                            href="{{ route('reports.student.student-reports') }}">{{ __('student_reports') }}</a></li>
                ...
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <div class="badge text-primary mb-3 font-weight-bold">
                            <span class="text-dark">{{ __('admission_no') }} : </span><span
                                class="text-dark">{{ $student->admission_no }}</span>
                        </div>
                        <hr>
                        <ul class="list-group list-group-flush text-left">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <span
                                    class="text-capitalize font-weight-medium">{{ $student->class_section->medium->name ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span><b>{{ __('class_section') }}:</b></span>
                                <span class="font-weight-medium">{{ $student->class_section->full_name ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span></i> <b>{{ __('Stream') }}:</b></span>
                                <span
                                    class="text-capitalize font-weight-medium">{{ $student->class_section->class->stream->name ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span><b>{{ __('Shift') }}:</b></span>
                                <span
                                    class="text-capitalize font-weight-medium">{{ $student->class_section->class->shift->name ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span><b>{{ __('session_year') }}:</b></span>
                                <span
                                    class="text-capitalize font-weight-medium">{{ $student->session_year->name ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span><b>{{ __('gender') }}:</b></span>
                                <span
                                    class="text-capitalize font-weight-medium">{{ ucfirst($student->user->gender) }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
                            ...
                            <div class="tab-pane fade show active py-3" id="profile" role="tabpanel">
                                <div class="card">
                                    ...
                                    <div class="card-body p-2">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <p><strong>{{ __('admission_date') }}:</strong>
                                                    {{ $student->admission_date ?? '-' }}</p>
                                                <p><strong>{{ __('dob') }}:</strong> {{ $student->user->dob ?? '-' }}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <p><strong>{{ __('mobile_number') }}:</strong>
                                                    {{ $student->user->mobile ?? '-' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body p-2">
                                        @if(isset($student->guardian))
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <p><strong>{{ __('name') }}:</strong> {{ $student->guardian->first_name }}
                                                        {{ $student->guardian->last_name }}</p>
                                                    <p><strong>{{ __('gender') }}:</strong>
                                                        {{ ucfirst($student->guardian->gender) }}</p>
                                                </div>
                                                <div class="col-md-6">
                                                    <p><strong>{{ __('email') }}:</strong> {{ $student->guardian->email }}</p>
                                                    <p><strong>{{ __('mobile_number') }}:</strong>
                                                        {{ $student->guardian->mobile }}</p>
                                                </div>
                                            </div>
                                        @else

    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderExamData(data) {
            ...
            if (data.success && data.exams && data.exams.length > 0) {
                ...
                // Render Offline Exams
                if (offlineExams.length > 0) {
                    offlineExams.forEach(exam => {
                        resultsWrapper.appendChild(createExamCard(exam));
                    });
                }

                // Render Online Exams
                if (onlineExams.length > 0) {
                     resultsWrapper.appendChild(createOnlineExamConsolidatedCard(onlineExams));
                } else {
                    // Show message if no online exams found for the session year
                    const msg = document.createElement('div');
                    msg.className = 'alert alert-info text-center';
                    msg.innerText = "{{ __('No online exam records found for the selected session year.') }}";
                ...
                // If data.exams is empty, it means no exams at all (online or offline)
                document.getElementById('no_exam_data').style.display = 'block';
                document.getElementById('no_exam_data').innerText = "{{ __('No exam records found for the selected session year.') }}";
            }
        }

        // Function to create consolidated online exam card
        function createOnlineExamConsolidatedCard(exams) {
            const card = document.createElement('div');
            card.className = 'card mb-4 border-primary';

            const cardHeader = document.createElement('div');
            cardHeader.className = 'card-header bg-primary theme-color text-white';
            cardHeader.innerHTML = '<h5 class="mb-0">{{ __("Online Exams") }}</h5>';
            ...
            tableEl.className = 'table table-striped table-bordered mb-0';
            ...
            thead.innerHTML = `
                <tr class="bg-light">
                    ...
                    <th class="text-center">{{ __("Date Taken") }}</th>
                </tr>
            `;
            ...
            exams.forEach(exam => {                
                ...
                row.innerHTML = `
                    <td>
                        ...
                        ${exam.subject_type ? `<br><span class="badge badge-${getSubjectTypeBadgeClass(exam.subject_type)}">${exam.subject_type}</span>` : ''}
                    </td>
                    ...
                    <td class="text-center">${Math.round(exam.percentage || 0)}%</td>
        ...
        // Function to create exam card (Offline Exams)
        function createExamCard(exam) {
            ...
            cardHeader.style.backgroundColor = 'rgba(40, 167, 69, 0.1)';
        ...
        `;
            ...
            if (exam.subjects && exam.subjects.length > 0 && exam.summary) {

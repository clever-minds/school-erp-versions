...
<nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
    <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
        <a class="navbar-brand brand-logo" href="{{ URL::to('/') }}">
            <img src="{{ $schoolSettings['horizontal_logo'] ?? $systemSettings['horizontal_logo'] ?? '' }}" alt="logo" data-custom-image="{{asset('/assets/horizontal-logo2.svg')}}  " class="custom-default-image" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1633332755192-727a05c4013d?auto=format&fit=crop&q=80&w=2080&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D';">
        </a>
        <a class="navbar-brand brand-logo-mini" href="{{ URL::to('/') }}">
            <img src="{{ $schoolSettings['vertical_logo'] ?? $systemSettings['vertical_logo'] ?? '' }}" alt="logo" data-custom-image="{{asset('/assets/vertical-logo.svg')}}  ">
        </a>
    </div>
    <div class="navbar-menu-wrapper d-flex align-items-stretch">
        ...
        <ul class="navbar-nav navbar-nav-right">
            ...
            @endcan
                ...
                <li class="nav-item">
                    <div class="text-dark">{{ __('Session Year') . ' : '}} <span id="sessionYearNameHeader">{{$sessionYear->name}}</span><span id="semesterNameHeader">{{ (isset($semester) ? ', '.$semester->name : null)}}</span></div>
                </li>
            @endif

...
<script>
    ...
    $('.login-redirect').click(function (e) {
        ...
        if (loginType === 'staff') {
            ...
            let studentWebUrl = "{{ $schoolSettings['student_web_url'] ?? '' }}";
            const schoolCode = "{{ $school->code ?? '' }}";

            if (studentWebUrl) {
                // Append school code to URL if it exists
                if (schoolCode) {
                    const separator = studentWebUrl.includes('?') ? '&' : '?';
                    studentWebUrl += separator + 'code=' + schoolCode;
                }

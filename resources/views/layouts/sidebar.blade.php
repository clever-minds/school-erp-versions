...
<nav class="sidebar sidebar-offcanvas" id="sidebar">
    <ul class="nav">
        ...
        @canany(['payroll-create', 'payroll-list', 'payroll-edit', 'payroll-delete'])
            <li class="nav-item">
                <a href="{{ route('payroll.index') }}" class="nav-link" data-name="{{ Auth::user()->getRoleNames()[0] }}" data-access="@hasFeatureAccess('Expense Management')">
        ...
        @if (Auth::user()->school_id)
            @canany(['role-list', 'role-create', 'role-edit', 'role-delete', 'staff-list', 'staff-create', 'staff-edit','staff-delete','approve-leave'])
                <li class="nav-item">
                ...
                <div class="collapse" id="settings-menu">
                    <ul class="nav flex-column sub-menu">
                        ...
                        @can('student-id-card-settings')
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('student.id-card-settings') }}" data-name="{{ Auth::user()->getRoleNames()[0] }}" data-access="@hasFeatureAccess('ID Card - Certificate Generation')">{{ __('student_id_card') }}</a>
                            </li>
                        @endcan

...
class SchoolDataService {
    ...
    public function createDatabaseMigration($schoolData)
    {
        ...
        $excludeMigration = '2024_08_08_094709_version1_4_1.php';
        $migrations = File::files(database_path('migrations'));
        

        foreach ($migrations as $migration) {
            // Skip the migration if it matches the excluded file
            if ($migration->getFilename() === $excludeMigration) {
                // $this->info("Skipping migration: {$migration->getFilename()}");
                continue;
            }

            // Run the migration for the current school's database
            // $this->info("Migrating: {$migration->getFilename()} for school: {$school->name}");

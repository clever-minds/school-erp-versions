...
class SchoolDataService
{
    ...
    public function preSettingsSetup($schoolData)
    {
        ...
        $school = School::withTrashed()->find($schoolData->id);
        if (!$school) {
            $school = new School();
        }
        ...
        $school->created_at = $schoolData->getRawOriginal('created_at');
        $school->updated_at = $schoolData->getRawOriginal('updated_at');
        $school->save();

        $mainUser = DB::connection('mysql')->table('users')->where('id', $schoolData->admin_id)->first();

        $user = User::on('school')->withTrashed()->find($mainUser->id);
    ...
    {
        $school_database = School::withTrashed()->where('id', $school_id)->pluck('database_name')->first();

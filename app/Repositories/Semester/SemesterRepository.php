...
class SemesterRepository extends SaaSRepository implements SemesterInterface {
    ...
    public function default($schoolId = null) {
        ...
        $sessionStartDate = date('Y-m-d', strtotime($defaultSessionYear->getRawOriginal('start_date')));
        $sessionEndDate = date('Y-m-d', strtotime($defaultSessionYear->getRawOriginal('end_date')));

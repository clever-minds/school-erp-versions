    ...
    public function index()
    {
        ...
        if (Auth::user()->hasRole('Super Admin') || Auth::user()->school_id == null) {
            $server_configuration = [
                'database_root_user' => [ 'title' => trans('database_root_user_access'), 'link' => 'https://wrteam-in.github.io/eSchool-SaaS-Doc/installation/admin-panel-installation/vps-server-setup/#9%EF%B8%8F%E2%83%A3-get-mysql-root-credentials','description' => trans('Required for creating and dropping school-specific databases in this SaaS system. When a new school is added, the root user permissions ensure database creation, migrations, and default setup run automatically.')],
                'laravel_queue_setup' => [ 'title' => trans('background_task_processing'), 'link' => 'https://wrteam-in.github.io/eSchool-SaaS-Doc/installation/admin-panel-installation/queue-setup/', 'description' => trans('The school creation process takes 2–3 minutes due to database setup and migrations. Laravel queues handle these tasks in the background so the system remains responsive.')],
                ...
                'web_socket_setup' => [ 'title' => trans('real_time_communication'), 'link' => 'https://wrteam-in.github.io/eSchool-SaaS-Doc/installation/admin-panel-installation/vps-server-setup/#-set-up-websocket', 'description' => trans('Required for mobile app real-time chat and instant updates. WebSockets ensure fast, bidirectional data exchange.')],

...
class PayrollController extends Controller
{
    ...
    public function show()
    {
        ...
        foreach ($res as $row) {
            ...
            if ($expense) {
                ...
                $operate = BootstrapTableService::button('fa fa-file-pdf', url('payroll/slip/' . $expense->id), ['btn-action-view'], ['title' => trans("slip"), 'target' => '_blank']);
    ...
    public function slip_list()
    {
        ...
        foreach ($res as $row) {
            $operate = BootstrapTableService::button('fa fa-file-pdf', url('payroll/slip/' . $row->id), ['btn-action-view'], ['title' => trans("slip"), 'target' => '_blank']);
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['operate'] = $operate;
            $rows[] = $tempRow;

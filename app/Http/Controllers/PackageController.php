    ...
    public function show()
    {
        ...
        foreach ($res as $row) {
            ...
            if (empty($showDeleted)) {
                if ($row->status == 0) {
                    $operate .= BootstrapTableService::menuButton('active', '#', ['change-package-status'], ['title' => __('active'), 'data-id' => $row->id]);
                } else {
                    $operate .= BootstrapTableService::menuButton('inactive', '#', ['change-package-status'], ['title' => __('inactive'), 'data-id' => $row->id]);
                }
                $operate .= BootstrapTableService::menuEditButton('edit', route('package.edit', $row->id), false);
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('package.destroy', $row->id), false);
            } else {
                $operate .= BootstrapTableService::menuRestoreButton('restore', route('package.restore', $row->id), false);
                $operate .= BootstrapTableService::menuTrashButton('trash', route('package.trash', $row->id), 'you_want_to_delete_this_record');
            }

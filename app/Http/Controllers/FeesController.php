    ...
    public function feesTransactionsLogsList(Request $request)
    {
        ...
        if ($request->session_year_id) {
            $sql = $sql->where(function ($query) use ($request) {
                $query->whereHas('compulsory_fees.fees_paid.fees', function ($q) use ($request) {
                    $q->where('session_year_id', $request->session_year_id);
                })->orWhereHas('optional_fees.fees_paid.fees', function ($q) use ($request) {
                    $q->where('session_year_id', $request->session_year_id);
                })->orWhereHas('transportation_payment', function ($q) use ($request) {
                    $q->where('session_year_id', $request->session_year_id);
                });
            });
        }
        ...
        if ($request->month) {
            $sql = $sql->whereRaw('MONTH(created_at) = ?', [$request->month]);
        }

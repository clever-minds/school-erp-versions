...
class NotificationController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = $this->user->guardian()->with('roles')->whereHas('child', function ($q) use ($sessionYearId) {
    ...
    public function userShow(Request $request)
    {
        ...
        $sql = $this->user->builder()->whereHas('roles', function ($q) {
            ...
            ->where(function ($q) use ($sessionYearId) {
                $q->whereHas('student', function ($q) use ($sessionYearId) {
                    $q->where('session_year_id', $sessionYearId);
                })->orWhereHas('child', function ($q) use ($sessionYearId) {
                    $q->where('session_year_id', $sessionYearId);
        ...
        if ($type == 'OverDueFees') {
            ...
            $fees = $this->fees->builder()->where('session_year_id', $sessionYearId)->whereDate('due_date', '<', $today)->get();
            ...
            if ($fees->isNotEmpty()) {
                foreach ($fees as $fee) {
                    $overdueStudents = $this->user->builder()
                        ...
                        ->whereHas('student.class_section', function ($q) use ($fee, $sessionYearId) {
                            $q->where('class_id', $fee->class_id)->where('session_year_id', $sessionYearId);
                        })

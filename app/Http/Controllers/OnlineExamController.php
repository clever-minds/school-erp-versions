...
class OnlineExamController extends Controller
{
    ...
    public function show()
    {
        ...
        // BASE QUERY: OnlineExamCommon 
        $sql = $this->onlineExamCommon->builder()
            ->with([
                'online_exam' => function ($q) use ($showDeleted) {
            ...
            ->whereHas('online_exam', function ($q) use ($showDeleted, $session_year_id, $exam_status) {
                // Determine whether to fetch active or only soft-deleted exams
                if (!empty($showDeleted)) {
                    $q->onlyTrashed();
                }
            ...
            ->when($search, function ($query) use ($search, $showDeleted) {
                $query->where(function ($query) use ($search, $showDeleted) {
                    $query->where('id', 'LIKE', "%$search%")
                        ->orWhereHas('online_exam', function ($q) use ($search, $showDeleted) {
                            if (!empty($showDeleted)) {
                                $q->onlyTrashed();
                            }
                            $q->where(function ($q2) use ($search) {
                                $q2->where('title', 'LIKE', "%$search%")
                                    ...
                                    ->orWhereHas('class_subject.subject', function ($q3) use ($search) {
        ...
        foreach ($res as $row) {
            ...
            if ($showDeleted) {
                ...
                $operate .= BootstrapTableService::menuTrashButton('delete', route('online-exam.trash', $examId), 'if_you_delete_this_exam_all_associated_student_results_will_also_be_permanently_deleted_this_action_cannot_be_undone');
            } else {

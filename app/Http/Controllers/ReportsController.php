...
class ReportsController extends Controller
{
    ...
    public function student_view_reports($id)
    {
        ...
        $promoted = PromoteStudent::where('student_id', $id)
        ...
        if ($transportationPayments) {
            ...
            $transportation = [
                'plan' => [
                    ...
                    'date' => $transportationPayments->paid_at_date_only,
        ...
        if (!$student) {
            return redirect('reports/student')->with('error', 'Student not found');
        }

        return view('reports.student.student-view-reports', compact('id', 'student', 'studentFees', 'sessionYears', 'sessionYear', 'session_year_id', 'transportation', 'attendanceMonths'));
    }
    ...
    public function getStudentDiaryReport(Request $request)
    {
        ResponseService::noPermissionThenRedirect('reports-student');
        ...
        $query = DiaryStudent::where('student_id', $request->student_id)
            ->with(['diary' => function ($q) {
                $q->with('diary_category', 'user', 'subject');
            }])
            ->whereHas('diary', function ($q) use ($request, $selectedSessionYearId) {
                $q->where('session_year_id', $selectedSessionYearId);

                if ($request->filled('month') && $request->filled('year')) {
                    $startOfMonth = Carbon::create($request->year, $request->month, 1)->startOfMonth()->format('Y-m-d');
                    $endOfMonth = Carbon::create($request->year, $request->month, 1)->endOfMonth()->format('Y-m-d');
                    $q->whereBetween('date', [$startOfMonth, $endOfMonth]);
                }

                if ($request->filled('type')) {
                    $type = $request->type;
                    $q->whereHas('diary_category', function ($q2) use ($type) {
                        $q2->where('type', $type);
                    });
                }
                ...
                if ($request->filled('search')) {
                    ...
                    $q->where(function ($q3) use ($search) {
                        $q3->where('description', 'like', "%$search%")
                            ->orWhereHas('diary_category', function ($q4) use ($search) {
        ...
        $query->join('diaries', 'diary_students.diary_id', '=', 'diaries.id')
            ->select('diary_students.*')
            ->orderBy('diaries.date', 'desc')
            ->orderBy('diaries.id', 'desc');

        // Compile Summary Counts from the filtered un-paginated query
        $summaryQuery = clone $query;
        // Strip the orderBy for a faster unpaginated get
        $summaryQuery->getQuery()->orders = null;

        // Remove eager loading for the summary query to eliminate N+1 overhead
        $summaryQuery->setEagerLoads([]);

        // Perform grouped aggregate query directly via Eloquent/Query Builder
        $stats = $summaryQuery->leftJoin('diary_categories', 'diaries.diary_category_id', '=', 'diary_categories.id')
            ->groupBy('diary_categories.type')
            ->select('diary_categories.type', \DB::raw('count(*) as aggregate'))
            ->get();

        $total = $stats->sum('aggregate');
        $positive = $stats->where('type', 'positive')->sum('aggregate');
        $negative = $stats->where('type', 'negative')->sum('aggregate');

        $diaries = $query->paginate(10);

        $diaries->getCollection()->transform(function ($ds) {
            $type = $ds->diary->diary_category->type ?? 'general';

            // Format Subject Tag
            $subjectLabel = null;
            if ($ds->diary->subject) {
                $subjectLabel = $ds->diary->subject->name;
                if ($ds->diary->subject->type) {
                    $subjectLabel .= ' - ' . ucfirst($ds->diary->subject->type);
                }
            }
            ...
            return [
                ...
                'date' => $ds->diary->date,
                'type' => $type,
                'title' => $ds->diary->title ?? 'Untitled',
                'description' => $ds->diary->description,
                'category_name' => $ds->diary->diary_category->name ?? 'Uncategorized',
                'subject_label' => $subjectLabel,
                'created_by' => $ds->diary->user ? trim($ds->diary->user->first_name . ' ' . $ds->diary->user->last_name) : 'System',
            ];
        });
    ...
    public function getStudentExamReport(Request $request)
    {
        ...
        if ($sessionYearId == $defaultSessionYear->id) {
            // Get student information including class section
            $student = $this->student->builder()
                ->where('user_id', $request->student_id)
            ...
            // Get data from promote student
            $student = PromoteStudent::where('user_id', $request->student_id)
                ->where('session_year_id', $sessionYearId)
                ->with('student.user', 'class_section.class', 'class_section.class.shift', 'class_section.section')
                ->first();
        ...
        foreach ($res as $row) {
            ...
            if (Auth::user()->can('exam-result-edit')) {
                $operate .= BootstrapTableService::button(
                    ...
                    ['btn-gradient-info', 'btn-xs', 'btn-icon'],
        ...
        foreach ($res as $row) {
            ...
            if (Auth::user()->can('exam-result-edit')) {
                $operate .= BootstrapTableService::button(
                    ...
                    ['btn-action-view', 'btn-xs', 'btn-icon'],
                    ['title' => __('view_result'), 'target' => '_blank']
                );
            }
            ...
            if (isset($row->vehicle->name) && isset($row->vehicle->vehicle_number)) {
                ...
                if ($fileExtension) {
                    if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'])) {
                        $previewHtml = '
                    <a href="' . $fileUrl . '" target="_blank" class="btn btn-xs btn-action-view">
                        <i class="fa fa-eye"></i>
                    </a>';
    ...
    public function teacher_view_reports($id)
    {
        ...
        $teacher = $this->user->builder()
            ...
            ->with(['staff' => function ($query) use ($sessionYear) {
                $query->with(['staffSalary' => function ($q) use ($sessionYear) {
                    $q->where('session_year_id', $sessionYear->id);
                }])
                    ->with(['class_teacher' => function ($q) use ($sessionYear) {
                        $q->where('session_year_id', $sessionYear->id);
                    }])
                    ->with(['subjects' => function ($q) use ($sessionYear) {
                        $q->where('session_year_id', $sessionYear->id)
        ...
        if ($transportationPayments) {
            ...
            $transportation = [
                'plan' => [
                    'plan_status' => ($transportationPayments->getRawOriginal('expiry_date') && $transportationPayments->getRawOriginal('expiry_date') >= date('Y-m-d')) ? 'Active' : 'Expired',
                    'vehicle_assignment' => ($transportationPayments->routeVehicle) ? 'Assigned' : 'Pending',
                    'expiry_date' => $transportationPayments->expiry_date,
                    'paid_amount' => ($schoolSettings['currency_symbol'] ?? '$') . " " . $transportationPayments->amount,
                    'payment_mode' => $transportationPayments->paymentTransaction->payment_gateway ?? null,
        ...
        // Get all transportation payment logs for history tab
        $transportationLogs = $this->getTransportationLogs($id, $schoolSettings);

        return view('reports.teacher.teacher-view-reports', compact('id', 'teacher', 'transportation', 'timetables', 'timetableSettingsData', 'salary_structure', 'sessionYear', 'attendanceMonths', 'transportationLogs', 'schoolSettings'));
    }
    ...
    public function teacherLeaves(Request $request)
    {
        ...
        $leaves = Leave::where('user_id', $teacherId)
            ->whereHas('leave_master', function ($q) use ($sessionYear) {
                $q->where('session_year_id', $sessionYear->id);
            })
            ->where(function ($q) use ($startDate, $endDate) {
                $q->whereBetween('from_date', [$startDate, $endDate])
    ...
    public function staff_reports()
    {
        ...
        return view('reports.staff.staff-reports', compact('extraFields'));
    }


    public function staff_reports_show()
    {
        $offset = request('offset', 0);
        $limit = request('limit', 10);
        $sort = request('sort', 'id');
        ...
        $sql = User::withTrashed()
            ->whereDoesntHave('roles', function ($q) {
                $q->whereIn('name', ['Teacher', 'Student', 'School Admin', 'Guardian']);
            })
            ->has('staff')
        ...
        foreach ($res as $row) {
            ...
            $operate = BootstrapTableService::viewButton(
                route('reports.staff.staff-view-reports', [$row->id]),
                [],
                [
                    'title' => trans('View Staff Details'),
                    'target' => ''
                ]
            );
    ...
    public function staff_view_reports($id)
    {
        $schoolSettings = $this->cache->getSchoolSettings();
        $sessionYear = $this->cache->getSessionYear();
        ...
        $userID = $user->id;
        $timetables = $this->timetable->builder()->whereHas('subject_teacher', function ($q) use ($userID) {
        ...
        $salaries = StaffSalary::where('staff_id', $user->staff->id)
            ->with('payrollSetting')
            ->get();

        $basic_salary = $user->staff->salary;
        ...
        return view('reports.staff.staff-view-reports', compact('id', 'user', 'transportation', 'timetables', 'timetableSettingsData', 'salary_structure', 'sessionYear', 'attendanceMonths', 'transportationLogs', 'schoolSettings'));
    }
    public function transportationLogsShow(Request $request, $userId)
    {
        $schoolSettings = app(CachingService::class)->getSchoolSettings();
        $currencySymbol = $schoolSettings['currency_symbol'] ?? '$';
        $timeFormat = $schoolSettings['time_format'] ?? 'H:i A';

        $offset = $request->offset ?? 0;
        $limit = $request->limit ?? 10;
        $sort = $request->sort ?? 'expiry_date';
        $order = $request->order ?? 'DESC';
        $sessionYear = $this->cache->getSessionYear();

        $query = TransportationPayment::with([
            'transportationFee',
            'paymentTransaction',
            'pickupPoint',
            'routeVehicle.route',
        ...
        if ($request->search) {
            ...
            $query->where(function ($q) use ($search) {
                $q->whereHas('routeVehicle.route', function ($qr) use ($search) {
                    $qr->where('name', 'LIKE', "%$search%");
                })->orWhereHas('pickupPoint', function ($qp) use ($search) {
                    $qp->where('name', 'LIKE', "%$search%");
                })->orWhereHas('routeVehicle.vehicle', function ($qv) use ($search) {
        ...
        foreach ($res as $row) {
            ...
            $isActive = $row->getRawOriginal('expiry_date') && $row->getRawOriginal('expiry_date') >= date('Y-m-d');
            $planStatus = $row->status === 'paid' ? ($isActive ? 'Active' : 'Expired') : 'Pending';
            $vehicleRoute = '';

            $vehicleRoute = ($row->routeVehicle->vehicle->name ?? '<i>N/A</i>');
            if ($row->routeVehicle->vehicle->vehicle_number ?? null) {
                $vehicleRoute .= ' <small class="text-muted">(' . ($row->routeVehicle->vehicle->vehicle_number ?? '') . ')</small>';
            }

            if ($planStatus == 'Active') {
                $vehicleRoute .= ' <span class="badge badge-success ml-1">' . __('active') . '</span>';
            }


            $vehicleRoute .= '<div><small class="text-muted">' . ($row->routeVehicle->route->name ?? '<i>N/A</i>') . '</small></div>';

            $paymentMode = '-';
            if ($row->paymentTransaction->payment_gateway ?? null) {
                $paymentMode = '<span class="badge badge-info"><i class="fa fa-credit-card"></i> ' . $row->paymentTransaction->payment_gateway . '</span>';
            }

            $period = '<span style="font-size: 12px; color: #64748b; white-space: nowrap;">' . $row->paid_at_date_only . '<div class="mt-1">' . ($row->expiry_date ?? '-') . '</div></span>';

            $operate = '<a href="' . route('transportation-requests.fee-receipt', $row->id) . '" class="btn btn-sm btn-info" target="_blank" data-toggle="tooltip" title="' . __('download_invoice') . '"><i class="fa fa-download"></i></a>';

            $rows[] = [
                'id'           => $row->id,
                'no'           => $no++,
                'vehicle_route' => $vehicleRoute,
                'pickup_point' => $row->pickupPoint->name ?? '-',
                'shift'        => $row->shift->name ?? '-',
                'duration'     => '<span style="text-transform: capitalize;">' . (optional($row->transportationFee)->duration ?? '-') . '</span>',
                'amount'       => '<span class="tlog-amount">' . $currencySymbol . ' ' . number_format($row->amount ?? 0, 2) . '</span>',
    ...
    private function getTransportationLogs(int $userId, mixed $schoolSettings): array
    {
        $currencySymbol = $schoolSettings['currency_symbol'] ?? '$';
        $timeFormat = $schoolSettings['time_format'] ?? 'H:i A';

        $payments = TransportationPayment::with([
            'transportationFee',
            'paymentTransaction',
            'pickupPoint',
            'routeVehicle.route',
        ...
        foreach ($payments as $payment) {
            $routePickupPoint = null;
            if ($payment->pickupPoint && $payment->routeVehicle?->route) {
                $routePickupPoint = RoutePickupPoint::where('pickup_point_id', $payment->pickupPoint->id)
                    ->where('route_id', $payment->routeVehicle->route->id)
                    ->first();
            }

            $isActive = $payment->expiry_date && $payment->expiry_date >= date('Y-m-d');

            $logs[] = [
                'id'                => $payment->id,
                'date'              => $payment->created_at?->format('d M Y') ?? '-',
                'expiry_date'       => $payment->expiry_date ?? '-',
                'amount'            => $currencySymbol . ' ' . number_format($payment->amount ?? 0, 2),
                'raw_amount'        => $payment->amount ?? 0,
                'status'            => ucfirst($payment->status ?? 'unknown'),
                'plan_status'       => $payment->status === 'paid' ? ($isActive ? 'Active' : 'Expired') : 'Pending',
                'route_name'        => $payment->routeVehicle->route->name ?? '-',
                'pickup_point_name' => $payment->pickupPoint->name ?? '-',
                'vehicle_name'      => $payment->routeVehicle->vehicle->name ?? '-',
                'vehicle_number'    => $payment->routeVehicle->vehicle->vehicle_number ?? '-',
                'shift_name'        => $payment->shift->name ?? '-',
                'fee_duration'      => optional($payment->transportationFee)->duration ?? '-',
                'fee_amount'        => $currencySymbol . ' ' . number_format(optional($payment->transportationFee)->fee_amount ?? 0, 2),
                'payment_mode'      => $payment->paymentTransaction->payment_gateway ?? '-',
                'session_year_name' => $payment->sessionYear->name ?? '-',
                'pickup_time'       => $routePickupPoint?->pickup_time
                    ? Carbon::parse($routePickupPoint->pickup_time)->format($timeFormat)
                    : '-',
                'drop_time'         => $routePickupPoint?->drop_time
                    ? Carbon::parse($routePickupPoint->drop_time)->format($timeFormat)
                    : '-',
            ];
        }

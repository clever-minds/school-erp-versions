...
class ClassSchoolController extends Controller
{
        ...
        try {
            ...
            if (!empty($request->core_subject)) {
                foreach ($request->core_subject as $row) {
                    $semId = $semesterIncluded ? ($row['semester_id'] ?? null) : null;
                    $coreSubjects[] = array('class_id' => $id, 'type' => "Compulsory", 'subject_id' => $row['id'], "elective_subject_group_id" => null, "semester_id" => $semId);
                }
            }
            ...
            if (!empty($request->elective_subject_group)) {
                foreach ($request->elective_subject_group as $subjectGroup) {
                    $semId = $semesterIncluded ? ($subjectGroup['semester_id'] ?? null) : null;
                    ...
                    foreach ($subjectGroup['subject'] as $subject) {
                        $electiveSubject[] = array('class_id' => $id, 'type' => "Elective", 'subject_id' => $subject['id'], 'elective_subject_group_id' => $electiveSubjectGroup->id, "semester_id" => $semId);
                    }
                }
            }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            //     ResponseService::errorResponse("Cannot delete this class because it is associated with records (subjects, fees, exams, or students) in one or more academic sessions. Please remove those associations first.");
            // }
    ...
    public function trash($id)
    {
        ...
        try {
            ...
            if ($hasSubjects || $hasFees || $hasExams) {
                ResponseService::errorResponse("Cannot delete this class permanently because it is associated with subjects, fees, or exams in academic sessions.");
            }
    ...
    public function classSubjectUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            if (!empty($request->elective_subject_group)) {
                foreach ($request->elective_subject_group as $subjectGroup) {
                    $semId = ($subjectGroup['semester_id'] ?? null);
                    $electiveSubjectGroup = $this->electiveSubjectGroup->updateOrCreate(
                        ['id' => $subjectGroup['id'],],
                        [
                            'total_subjects' => count($subjectGroup['subject']),

...
class TrasportationApiController extends Controller
{
    ...
    public function __construct(HolidayInterface $holiday, StudentInterface $student, PickupPointRepositoryInterface $pickupPoint, SchoolSettingInterface $schoolSetting, CachingService $cache, PaymentTransactionInterface $paymentTransaction, ExpenseInterface $expense, SystemSettingInterface $systemSetting)
    {
    ...
    public function getTransportationData(Request $request)
    {
        ...
        try {
            ...
            // Build response data
            $differenceInDays = now()->diffInDays($transportationPayment->getRawOriginal('expiry_date'), false) + 2;
            $differenceInDaysForStaff = !$transportationPayment->transportationFee
                ? Carbon::parse($transportationPayment->getRawOriginal('paid_at'))->diffInDays($transportationPayment->getRawOriginal('expiry_date'), false)
                : null;
            ...
            $data = [
                'plan' => [
                    ...
                    'valid_from' => date("d F Y", strtotime($transportationPayment->getRawOriginal('paid_at'))),
                    'valid_to' => date("d F Y", strtotime($transportationPayment->getRawOriginal('expiry_date'))),
                    'route' => [
    ...
    public function getTransportationRequests(Request $request)
    {
        ...
        try {
                ...
                $data[] = [
                    ...
                    "details" => [
                        ...
                        "plan" => [
                            ...
                            "validity" => (!empty($transportationPayment->getRawOriginal('paid_at')) && !empty($transportationPayment->getRawOriginal('expiry_date')))
                                ? date("Y-m-d", strtotime($transportationPayment->getRawOriginal('paid_at'))) . " - " . date("Y-m-d", strtotime($transportationPayment->getRawOriginal('expiry_date')))
                                : "N/A",
                        ],
            ...
            ResponseService::successResponse("Transportation requests fetched successfully", $data);
        } catch (\Throwable $th) {
    ...
    public function getvehicleAssignmentstatus(Request $request)
    {
        ...
        try {
            ...
            if (!$transportationPayment) {
                ResponseService::successResponse("No request found", "false");
            }

            // Check expiry
            if ($transportationPayment->getRawOriginal('expiry_date') < $today) {
                ResponseService::successResponse("Plan expired", "expired");
            }

            // Check vehicle assignment
            if ($transportationPayment->route_vehicle_id === null) {
                ResponseService::successResponse("Vehicle assignment pending", "pending");
            }

            ResponseService::successResponse("Vehicle assigned", "assigned");
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);
            ResponseService::errorResponse();
        }
    ...
    public function getTransoprtationCurrentPlan(Request $request)
    {
        ...
        try {
            ...
            $plans = TransportationPayment::with([
                'pickupPoint',
                'paymentTransaction',
                'shift',
                'transportationFee',
            ...
            foreach ($plans as $key => $plan) {
                $userPickupPointId = optional($plan->pickupPoint)->id;
                $pickupPoint = $plan->routeVehicle?->route->routePickupPoints
                    ->where('pickup_point_id', $userPickupPointId)
                    ->first();

                $differenceInDays = now()->diffInDays($plan->getRawOriginal('expiry_date'), false) + 2;

                $data[] = [
                    'id' => $plan->id,
                    'payment_id' => $plan->id,
                    'shift_id' => $plan->shift_id,
                    'duration' => optional($plan->transportationFee)->duration . " Days",
                    'valid_from' => date("d F Y", strtotime($plan->getRawOriginal('paid_at'))),
                    'valid_to' => date("d F Y", strtotime($plan->getRawOriginal('expiry_date'))),
                    'expires_in_days' => $differenceInDays > 0 ? $differenceInDays : 0,
                    'total_fee' => ($settings['currency_symbol'] ?? '') . " " . optional($plan->transportationFee)->fee_amount,
                    'payment_mode' => optional($plan->paymentTransaction)->payment_gateway,
                    'plan_status' => $plan->plan_status,
                    'status' => $plan->status,
                    'route' => [
                        'id' => optional($plan->routeVehicle?->route)->id,
                        'name' => optional($plan->routeVehicle?->route)->name,
                    ],
                    'shift' => [
                        'name' => optional($plan->shift)->name,
                        'time_window' => (optional($plan->shift)->start_time && optional($plan->shift)->end_time)
                            ? date("h:i A", strtotime($plan->shift->start_time)) . " - " . date("h:i A", strtotime($plan->shift->end_time))
                            : null,
                    ],
                    'pickup_stop' => [
                        'id' => optional($plan->pickupPoint)->id,
                        'name' => optional($plan->pickupPoint)->name,
                    ],
                    'vehicle' => [
                        'vehicle_id' => optional($plan->routeVehicle)->vehicle_id,
                        'vehicle_name' => optional($plan->routeVehicle?->vehicle)->name,
                        'vehicle_registration' => optional($plan->routeVehicle?->vehicle)->vehicle_number,
                    ],
            ...
            ResponseService::successResponse("Plan fetched successfully", $data);
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);
            ResponseService::errorResponse();
        }
    ...
    public function transportationReceipt(Request $request)
    {
        ...
        try {
            ...
            $TransportationPayment = TransportationPayment::where('status', 'paid')->with('pickupPoint', 'transportationFee', 'paymentTransaction')->where('id', $request->id)->first();
            ...
            return view('transportation-request.fee_receipt', compact('school', 'TransportationPayment', 'user', 'schoolSettings', 'systemSettings'));
        } catch (Throwable $e) {

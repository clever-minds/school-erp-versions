...
class StaffApiController extends Controller
{
    ...
    public function myPayrollSlip(Request $request)
    {
        ...
        try {
            ...
            $pdf = PDF::loadView('payroll.slip',compact('vertical_logo','schoolSetting','salary','total_leaves','days'))->output();
    ...
    public function staffPayrollList(Request $request)
    {
        ...
        try {
            ...
            $sql = $this->staff->builder()->with(['user:id,first_name,last_name,image', 'expense:id,staff_id,basic_salary,paid_leaves,month,year,title,amount,date', 'leave' => function ($q) use ($month,$year) {
    ...
    public function leaveRequest(Request $request)
    {
        ...
        try {
            if ($request->leave_id) {
                $sql = $this->leave->findById($request->leave_id,['*'],['user:id,first_name,last_name,image,email,mobile','leave_detail']);
            } else {
                $sql = $this->leave->builder()->where('status',0)->with('user:id,first_name,last_name,image,email,mobile','leave_detail')->get();
            }
            ResponseService::successResponse('Data Fetched Successfully', $sql);
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);

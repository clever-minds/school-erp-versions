...
class StaffApiController extends Controller
{
    ...
    public function getTasks(Request $request)
    {
        try {
            ...
            $sql = Task::with('user:id,first_name,last_name,image,mobile,email', 'user.roles:id,name');

            if ($request->status && $request->status != "overdue") {
                $sql = $sql->where('status', $request->status);
            }

            if ($request->status == "overdue") {
                $sql = $sql->where('status', '!=', 'completed')
            ...
            if ($request->search) {
                $sql = $sql->where('title', 'like', '%' . $request->search . '%')
                    ->orWhere('description', 'like', '%' . $request->search . '%');
            }

            if ($request->type == 'staff') {
                $sql = $sql->whereNot('user_id', Auth::user()->id);
            } else if ($request->type == 'my_tasks') {
                $sql = $sql->where('user_id', Auth::user()->id);
            }
            ...
            // $sql = $sql->orderByRaw("CASE WHEN status = 'in_progress' THEN 1 WHEN status = 'pending' THEN 2 ELSE 3 END ASC")
            //     ->orderBy('due_date', 'DESC')->paginate($request->page_size ?? 10);
            ...
            ResponseService::successResponse('Tasks Fetched Successfully', $sql);
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e, "Staff API Controller -> getTasks Method");
            ResponseService::errorResponse();
        }
    }
    ...
    public function storeTask(Request $request)
    {
        try {
            ...
            ResponseService::logErrorResponse($e, "Staff API Controller -> storeTask Method");
            ResponseService::errorResponse();
        }
    }
    ...
    public function updateTask(Request $request)
    {
        try {
            ...
            $task->user_id = $request->user_id;
            $task->title = $request->title;
            if ($request->status && $task->status == "completed") {
            ...
            ResponseService::successResponse('Task Updated Successfully', $task);
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e, "Staff API Controller -> updateTask Method");
            ResponseService::errorResponse();
        }
    }
    ...
    public function deleteTask(Request $request)
    {
        try {
            ...
            ResponseService::successResponse('Task Deleted Successfully', $task);
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e, "Staff API Controller -> destroyTask Method");
            ResponseService::errorResponse();
        }
    }
    ...
    public function updateTaskStatus(Request $request)
    {
        try {
            ...
            ResponseService::successResponse('Task Status Updated Successfully', $task);
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e, "Staff API Controller -> updateTaskStatus Method");
            ResponseService::errorResponse();
        }
    }

    public function getUsersRoleWise(Request $request)
    {
        try {

            $users = User::where('status', 1)->whereHas('roles', function ($q) {
                $q->whereNotIn('name', ['School Admin', 'Student', 'Guardian']);
            })->select('id', 'first_name', 'last_name', 'image', 'mobile', 'email');
            ...
            $users = $users->with('roles:id,name')->paginate($request->page_size ?? 10);

            ResponseService::successResponse('Users Fetched Successfully', $users);
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e, "Staff API Controller -> getUsersRoleWise Method");
            ResponseService::errorResponse();
        }
    }
}

...
class TeacherApiController extends Controller
{
  ...
  Log::info('Subject ID received in request', [
    ...
    public function getStudentDiaries(Request $request)
    {
        ...
        try {
            $sortType = strtolower($request->get('sort_type', 'Newest first'));
            switch ($sortType) {
                case 'oldest first':
                    $sortBy = 'created_at';
                    $sortDirection = 'asc';
            ...
            $users = $this->user->builder()->select('id', 'first_name', 'last_name', 'mobile', 'email', 'image', 'dob')->has('diary_student');
            ...
            if($request->student_id){
                $users = $users->where('id', $request->student_id);
            }

            if($request->type){
                $users = $users->whereHas('diary_student', function($q) use ($request){
                    $q->whereHas('diary.diary_category', function($q) use ($request){
                        $q->where('type', $request->type);
                    });
                })
                ->with(['diary_student' => function($q) use ($sortBy, $sortDirection, $request) {
                    $q->orderBy($sortBy, $sortDirection)
                      ->with(['diary' => function($q2) use ($request) {
                          $q2->whereHas('diary_category', function($q3) use ($request){
                              $q3->where('type', $request->type);
                          });
                          $q2->with('diary_category', 'subject');
                      }]);
                }]);
            } else {
                $users = $users->with(['diary_student' => function($q) use ($sortBy, $sortDirection) {
                    $q->orderBy($sortBy, $sortDirection)
                      ->with('diary.diary_category', 'diary.subject');
                }]);
            }

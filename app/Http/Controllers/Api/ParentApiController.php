...
class ParentApiController extends Controller
{
                ...
                if (count($fee->installments) > 0) {
                    ...
                    foreach ($fee->installments as $installment) {
                        ...
                        if (!empty($installmentPaid)) {
                            ...
                            if (intval($installment->installment_amount) == intval($installmentPaid->amount)) {
                            ...
                            $installment['paid_amount'] = $installmentPaid->amount;
                            $installment['due_charges_amount'] = $installmentPaid->due_charges;
                            $totalFeesPaidAmount += $installmentPaid->amount;
            ...
            if (isset($request->installment_ids) && count($request->installment_ids)) {
                ...
                foreach ($validInstallmentIDS as $key => $installment_id) {
                    ...
                    if (new DateTime(date('Y-m-d')) > new DateTime($installment['due_date'])) {
                        if ($installment->due_charges_type == "percentage") {
                            $dueChargesAmount = ($installment['installment_amount'] * $installment['due_charges']) / 100;
                        } else if ($installment->due_charges_type == "fixed") {
                            $dueChargesAmount = $installment->due_charges;
                        }
                        $amount += $installment['installment_amount'] + $dueChargesAmount;
                    } else {
                        $dueChargesAmount = 0;
    ...
    public function feesPaidReceiptPDF(Request $request)
    {
        ...
        try {
            ...
            $feesPaid = $this->feesPaid->builder()->where('student_id', $student->user_id)->where('fees_id', $request->fees_id)
                ->with([
    ...
    public function getSliders(Request $request)
    {
        ...
        try {
            ...
            $data = $this->sliders->builder()->where('school_id', $child->user->school_id)->whereIn('type', [1, 3])->get();

...
class ApiController extends Controller
{
    ...
    public function getHolidays(Request $request)
    {
        try {
            ...
            $sessionyearStartDate = carbon::createFromFormat($settings['date_format'] ?? 'Y-m-d', $sessionYear->start_date)->format('Y-m-d');
            $sessionyearEndDate = carbon::createFromFormat($settings['date_format'] ?? 'Y-m-d', $sessionYear->end_date)->format('Y-m-d');
        ...
        try {
            ...
            if ($request->type == "app_settings") {
                $data = array(
                    ...
                    'system_maintenance' => $systemSettings['web_maintenance'] ?? "",
    ...
    public function getSystemSettings(Request $request)
    {
        try {
            ...
            if ($school_code) {
                ...
                ResponseService::successResponse("System Settings Fetched Successfully", $data);
            }
        } catch (Throwable $e) {
            ResponseService::logErrorResponse($e);
            ResponseService::errorResponse();
    ...
    public function getCertificates(Request $request)
    {
        try {
            $user = Auth::user();
            if ($user && $user->HasRole('Student') || $user->HasRole('Guardian')) {
                $rows = CertificateTemplate::where('type', 'student');
            } else {
                $rows = CertificateTemplate::where('type', 'Staff');
            }

            if ($request->session_year_id) {
                $rows = $rows->where('session_year_id', $request->session_year_id);
            }
            $rows = $rows->select('id', 'name', 'type')->paginate($request->limit ?? 10);
            ResponseService::successResponse("Certificates Fetched Successfully", $rows);
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);
            ResponseService::errorResponse();
        }
    ...
    public function certificateAssign(Request $request)
    {

        try {
            $user = Auth::user();
            if ($user && $user->hasRole('Student')) {
                $rows = CertificateAssignment::where('user_id', $user->id);
            } else if ($user && $user->hasRole('Guardian')) {
                // add validation if user_id is not present
                if (!$request->user_id) {
                    ResponseService::errorResponse('Child user id is required');
                }
                $rows = CertificateAssignment::where('user_id', $request->user_id);
            } else {
                $rows = CertificateAssignment::where('user_id', $user->id);
            }
            ...
            if ($request->certificate_template_id) {
                $rows = $rows->where('certificate_template_id', $request->certificate_template_id);
            }

            $rows = $rows->with('user:id,first_name,last_name,email,mobile,image', 'exam', 'certificate_template:id,name,type')->get();
            ResponseService::successResponse("Certificate Assignments Fetched Successfully", $rows);
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);
            ResponseService::errorResponse();
        }
    ...
    public function certificateGenerate(Request $request)
    {
        ...
        try {
            ...
            if (!$certificateTemplate) {
                ...
                $certificateTemplate = CertificateTemplate::find($request->certificate_template_id);
            }
            ...
            $settings = $this->cache->getSchoolSettings('*', $assignments->first()->user->school_id);
    ...
    public function getStudentIdCard(Request $request)
    {
        try {
            ...
            if (!isset($settings['student_id_card_fields'])) {
                ResponseService::successResponse('settings_not_found');
            }
            ...
            $students = User::select('id', 'first_name', 'last_name', 'image', 'school_id', 'gender', 'dob')->with('student', 'student.class_section.class', 'student.class_section.class.shift', 'student.class_section.section', 'student.class_section.medium', 'student.class_section.class.stream', 'student.guardian:id,mobile,first_name,last_name')->with([
            ...
            ResponseService::successResponse("Certificates Fetched Successfully");
        } catch (\Throwable $th) {
            ResponseService::logErrorResponse($th);
            ResponseService::errorResponse();
        }
    ...
    public function getStaffIdCard(Request $request)
    {
        try {
            $user = Auth::user();
            $user_ids[] = $user->id;

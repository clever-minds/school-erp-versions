...
class ExamController extends Controller {
    ...
    public function show() {
        ...
        $sql = $this->exam->builder()->with('class.medium', 'class.stream', 'timetable')
...
//            ->get();
    ...
    public function marksList(Request $request) {
        ...
        $exam = $this->exam->builder()->with('timetable:id,date')
        ...
        // Get Students Data on the basis of Student ids
        $students = $this->users->builder()->role('Student')->whereIn('id',$studentIds)
                ->with(['exam_marks' => function($query) use($timetable){
    ...
    public function submitMarks(Request $request) {
        ...
        $request->validate([
            ...
            'class_id.required'            => 'Class section field is required.',
            'exam_id.required'             => 'Exam field is required.',
            'class_subject_id.required'    => 'Subject field is required.',
            'exam_marks.required'          => 'No records found.',
        ]);
        ...
        try {
            ...
            foreach ($request->exam_marks as $examMarks) {
                ...
                $this->examMarks->updateOrCreate(
                    ['id' => $examMarks['exam_marks_id'] ?? null],
                    [
    ...
    public function getExamResultIndex() {
        ...
        return view('exams.show_exam_result', compact('exams', 'sessionYears'));
    }
    ...
    public function showExamResult(Request $request) {
        ...
        if (!$request->exam_id) {
            ResponseService::errorResponse("Exam ID is Required");
        }
        $offset = request('offset', 0);
        $limit = request('limit', 10);
        $sort = request('sort', 'id');
        ...
        if (Auth::user()->hasRole('Teacher')) {
            $teacher_id = Auth::user()->teacher->user_id;
            $classSectionId = $this->classSection->builder()->whereHas('class_teachers', function ($query) use ($teacher_id) {
                $query->where('teacher_id', $teacher_id);
            })->pluck('id')->first();
            $examTimetableId = $this->examTimetable->builder()->where('exam_id', $request->exam_id)->pluck('id');

            $sql = $this->examResult->builder()->with('user:id,first_name,last_name', 'session_year:id,name')->with([
                'user.exam_marks' => function ($q) use ($examTimetableId) {
                    $q->whereIn('exam_timetable_id', $examTimetableId)->with('timetable', 'class_subject.subject');
                }
            ])->where(['exam_id' => $request->exam_id, 'class_section_id' => $classSectionId])
            ...
            $sql = $this->examResult->builder()->with('user:id,first_name,last_name', 'session_year:id,name')->with([
                'user.exam_marks' => function ($q) use ($examTimetableId) {
                    $q->whereIn('exam_timetable_id', $examTimetableId)->with('timetable', 'class_subject.subject:id,name');
                }

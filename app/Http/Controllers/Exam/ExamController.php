    ...
    public function marksList(Request $request)
    {
        ...
        try {
            ...
            foreach ($students as $no => $student) {
                $rows[] = ['id' => $student->id, 'no' => $no + 1, 'student_name' => $student->full_name, 'total_marks' => $timetable->total_marks, 'exam_marks_id' => $student->exam_marks[0]->id ?? '', 'obtained_marks' => $student->exam_marks[0]->obtained_marks ?? '', 'status' => $student->exam_marks[0]->status ?? null, 'operate' => '<a href=' . route('exams.edit', $student->id) . ' class="btn btn-xs btn-gradient-primary btn-icon edit-data" data-id=' . $student->id . ' title="Edit" data-toggle="modal" data-target="#editModal"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;<a href=' . route('exams.destroy', $student->id) . ' class="btn btn-xs btn-gradient-danger btn-icon delete-form" data-id=' . $student->id . '><i class="fa fa-trash"></i></a>',];
            }
    ...
    public function showExamResult(Request $request)
    {
        ...
        foreach ($res as $row) {
            ...
            if (Auth::user()->can('exam-result-edit')) {
                $operate = BootstrapTableService::button('fa fa-edit', '#', ['btn-action-edit', 'btn-xs', 'btn-icon', 'edit-data'], ['data-id' => $row->id, 'data-student_id' => $row->student_id, 'title' => 'Edit', 'data-toggle' => 'modal', 'data-target' => '#editModal']);

                $operate .= BootstrapTableService::button('fa fa-file-pdf', url('exams/result/student/') . '/' . $row->student_id . '/exam/' . $row->exam_id, ['btn-action-view', 'btn-xs', 'btn-icon',], ['title' => __('view_result'), 'target' => '_blank']);
            }
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            ...
            if ($exam->exam_status == 2 && $exam->marks->isNotEmpty()) {
                ...
                if ($exam->publish == 0) {
                    ...
                    $examResult = $exam->marks->map(function ($examMarks) use ($exam, $id) {
                        ...
                        $data = [
                            'exam_id' => $exam->id,
                            'class_section_id' => $examMarks['user']['student']['class_section_id'] ?? null,
                    ...
                    $guardian_id = Students::with('user')->whereIn('user_id', $studentIds)->pluck('guardian_id')->toArray();

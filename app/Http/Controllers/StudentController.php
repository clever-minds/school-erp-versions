...
class StudentController extends Controller
{
    ...
    public function create()
    {
        ...
        $class_sections = $this->classSection->builder()
            ->with(['class', 'class.stream', 'section', 'medium', 'class.shift'])
    ...
    public function trash($id)
    {
        ...
        try {
            ...
            $student = Students::with('guardian')->where('user_id', $id)->first();

            if ($student && $student->guardian) {
                // Count total students with same guardian_id
                $guardianStudentCount = Students::where('guardian_id', $student->guardian_id)->count();

                // If guardian has exactly one student, delete the guardian
                if ($guardianStudentCount == 1) {
                    User::where('id', $student->guardian_id)->withTrashed()->forceDelete();
                }
            }

            // Delete student and user records
            Students::where('user_id', $id)->withTrashed()->forceDelete();
    ...
    public function storeBulkData(Request $request)
    {
        ...
        try {
            Excel::import(new StudentsImport($request->class_section_id, $request->session_year, $request->is_send_notification), $request->file);
    ...
    public function resetPasswordShow()
    {
        ...
        $no = 1;
        foreach ($res as $row) {
            $operate = BootstrapTableService::button('fa fa-edit', route('student.reset-password.update', $row->id), ['reset_password', 'btn-action-edit', 'btn-action', 'btn-icon'], ['title' => trans("reset_password"), 'data-id' => $row->id, 'data-dob' => $row->getRawOriginal('dob')]);
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
    ...
    public function onlineRegistrationList(Request $request)
    {
        ...
        $sql = $this->student->builder()->where('application_type', 'online')->where('application_status', 0)->orWhereNull('application_status')->with('user.extra_student_details.form_field', 'guardian', 'class.medium', 'class.stream', 'class.shift')
            ->where(function ($query) use ($search) {
                $query->when($search, function ($query) use ($search) {
                    $query->where(function ($query) use ($search) {
                        $query->where('user_id', 'LIKE', "%$search%")
    ...
    public function updateApplicationStatus(Request $request)
    {
        ...
        try {
            ...
            $student = Students::where('user_id', $request->edit_user_id)->first();
            if ($user->status == 0) {
                $subscription = $this->subscriptionService->active_subscription(Auth::user()->school_id);
                // If prepaid plan check student limit
                if ($subscription && $subscription->package_type == 0) {
            ...
            if ($request->application_status == 1) {
                ...
                if ($defaultSessionYear && $defaultSessionYear->id != $request->session_year_id) {
                    // create record in promote_students table
                    $data[] = [
                    ...
                    PromoteStudent::upsert($data, ['student_id', 'session_year_id', 'class_section_id'], ['school_id', 'status', 'result']);
                }

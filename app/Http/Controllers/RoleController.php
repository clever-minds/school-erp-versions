...
class RoleController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $no = 1;
        foreach ($res as $row) {
            $operate = BootstrapTableService::menuButton('view', route('roles.show', $row->id), [], ['title' => 'View'], 'fa fa-eye');
            if (Auth::user()->can('role-edit')) {
                $operate .= BootstrapTableService::menuEditButton('edit', route('roles.edit', $row->id), false);
            }
            if (Auth::user()->can('role-delete') && $row->custom_role != 0) {
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('roles.destroy', $row->id), false);
            }

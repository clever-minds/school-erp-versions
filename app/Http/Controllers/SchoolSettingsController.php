...
class SchoolSettingsController extends Controller
{
    ...
    public function __construct(SchoolSettingInterface $schoolSettings, CachingService $cachingService, ClassSectionInterface $classSection, StudentInterface $student, SchoolInterface $school, FormFieldsInterface $formField, ExtraSchoolDataInterface $extraSchoolData)
    {
    ...
    public function index()
    {
        ...
        $extraFields = $this->formField->builder()->with(['extra_school_data' => function ($query) {
            $query->where('school_id', Auth::user()->school_id);
        }])->whereNull('school_id')->orderBy('rank')->get();
        $schoolService->switchToSchoolDatabase(Auth::user()->school_id);

        $schoolBoard = SchBoard::where('status', 1)->with(['school_board' => function ($q) {
            $q->where('school_id', Auth::user()->school_id);
        }])->get();

        return view('school-settings.general-settings', compact('settings', 'getDateFormat', 'getTimeFormat', 'baseUrlWithoutScheme', 'systemSettings', 'domain_type', 'extraFields', 'schoolBoard'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $settings = [
            ...
            'login_page_logo' => 'nullable|image|mimes:jpeg,jpg,png|max:2048',
            ...
            'extra_fields.*.data' => 'nullable',
            'extra_fields.*.form_field_id' => 'required',
            'extra_fields.*.input_type' => 'required',
        ...
        try {
            ...
            foreach ($settings as $key => $rule) {
                if ($key == 'horizontal_logo' || $key == 'vertical_logo' || $key == 'favicon' || $key == 'login_page_logo' || $key == 'signature') {
                    ...
                    foreach ($extraFields as $fields) {
                        if ($fields['input_type'] == 'file') {
                            if (isset($fields['data']) && $fields['data'] instanceof UploadedFile) {
                                $image = UploadService::upload($fields['data'], 'extra_school_data');
                                $schoolDataArray[] = [
                                    'id' => $fields['id'] ?? null,
            ...
            if ($request->school_board_id) {
                $board[] = [
                    'school_id' => Auth::user()->school_id,
                    'sch_board_id' => $request->school_board_id
                ];
    ...
    public function id_card_store(Request $request)
    {
        ...
        try {
            ...
            $data = array();
            foreach ($settings as $key => $rule) {
                if ($key == 'background_image' || $key == 'staff_background_image') {
                    if ($request->hasFile($key)) {
                        // TODO : Remove the old files from server
                        $data[] = [
                            "name" => $key,

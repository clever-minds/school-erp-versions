...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->payment_method == 'stripe') {
                return $this->subscriptionService->stripe_payment($request->id ?? null, $request->package_id, $request->type, null, null);
    ...
    public function show()
    {
        ...
        // Get active subscription for live counts
        ...
        foreach ($res as $row) {
            ...
            $operate = BootstrapTableService::button('fa fa-dollar', '#', ['btn', 'btn-xs', 'btn-action-edit', 'btn-icon', 'edit-data'], ['data-id' => $row->id, 'title' => trans('view_bill'), "data-toggle" => "modal", "data-target" => "#editModal"]);
            $operate .= BootstrapTableService::button('fa fa-file-pdf', url('subscriptions/bill/receipt', $row->id), ['btn-action-view'], ['title' => 'Receipt', 'target' => '_blank']);
            ...
            // Live counts for active plan
            if ($active_package && $row->subscription_id == $active_package->id) {
                DB::setDefaultConnection('school');
                if ($row->subscription->package_type == 1) {
                    $students = $this->user->builder()->withTrashed()->where(function ($q) use ($row) {
                        $q->whereBetween('deleted_at', [$row->subscription->start_date, $row->subscription->end_date]);
                    })->orWhereNull('deleted_at')->Owner()->role('Student')->count();

                    $staffs = $this->staff->builder()->whereHas('user', function ($q) use ($row) {
                        $q->where(function ($q) use ($row) {
                            $q->withTrashed()->whereBetween('deleted_at', [$row->subscription->start_date, $row->subscription->end_date])
                                ->orWhereNull('deleted_at');
                        })->Owner();
                    })->count();
                } else {
                    $students = $this->user->builder()->where('status', 1)->role('Student')->where('school_id', $row->subscription->school_id)->count();

                    $staffs = $this->staff->builder()->whereHas('user', function ($q) use ($row) {
                        $q->where('status', 1)->where('school_id', $row->subscription->school_id);
                    })->count();
                }
                $tempRow['total_student'] = $students;
                $tempRow['total_staff'] = $staffs;
                DB::setDefaultConnection('mysql');
            } else {
    ...
    public function subscription_report()
    {
        ...
        $boards = SchBoard::where('status', 1)->pluck('name', 'id');
        ...
        return view('schools.subscription', compact('data', 'settings', 'packages', 'boards'));
    }
    ...
    public function assign_plan(Request $request)
    {
        if (env('DEMO_MODE')) {
            return response()->json(array(
                'error' => true,
        ...
        try {
            ...
            // Create new active subscription plan
            $new_subscription = $this->subscriptionService->createSubscription($request->package_id, $request->school_id, null, 1);

            $this->cache->removeSchoolCache(config('constants.CACHE.SCHOOL.FEATURES'), $new_subscription->school_id);
            DB::commit();
            ResponseService::successResponse('Plan Assigned Successfully');
        } catch (Throwable $e) {
            DB::rollBack();
            ResponseService::logErrorResponse($e);
            ResponseService::errorResponse();
    ...
    public function subscription_report_schools_show(Request $request)
    {
        ResponseService::noPermissionThenRedirect('subscription-view');
        $offset = request('offset', 0);
        $limit = request('limit', 10);
        ...
        $sql = $this->school->builder()->with(['user', 'subscription' => function ($q) {
        ...
        if ($board_id) {
            $sql = $sql->whereHas('school_board', function ($query) use ($board_id) {
                $query->where('sch_board_id', $board_id);
            });
        }
        ...
        foreach ($res as $row) {
            ...
            $operate = '<a href="' . route('school.subscription.detail', $row->id) . '" class="btn btn-sm btn-primary text-center rounded-pill btn-icon-text">' . __('view_report') . '</a>';

            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['school_name'] = $row->name;
            $tempRow['school_email'] = $row->support_email;
            $tempRow['logo'] = $row->logo;

            if ($activeSubscription) {
                $tempRow['current_plan'] = $activeSubscription->name;
                $tempRow['status'] = $activeSubscription->status;
                $tempRow['next_billing'] = $activeSubscription->bill_date;
                $tempRow['package_type'] = $packageType;
            } else {
                $tempRow['current_plan'] = 'None';
                $tempRow['status'] = 'Inactive';
                $tempRow['next_billing'] = '-';
                $tempRow['package_type'] = $packageType;
            }
            $tempRow['operate'] = $operate;
            $rows[] = $tempRow;
    ...
    public function school_subscription_detail($id)
    {
        ResponseService::noPermissionThenRedirect('subscription-view');

        $school = $this->school->builder()->with(['subscription' => function ($q) {
            $q->with('subscription_bill.transaction')->orderBy('end_date', 'DESC');
        }])->findOrFail($id);

        $activeSubscription = null;
        $today = Carbon::now()->format('Y-m-d');
        foreach ($school->subscription as $sub) {
            if ($sub->start_date <= $today && $sub->end_date >= $today) {
                $activeSubscription = $sub;
                break;
            }
        ...
        $totalSpent = 0;
        foreach ($school->subscription as $sub) {
            if ($sub->subscription_bill && $sub->subscription_bill->amount) {
                // include paid only or all? UI says Total Spent, meaning fully paid invoices.
                if ($sub->subscription_bill->transaction && $sub->subscription_bill->transaction->payment_status == 'succeed') {
                    $totalSpent += $sub->subscription_bill->amount;
        ...
        return view('schools.subscription_detail', compact('school', 'activeSubscription', 'totalSpent', 'settings', 'packages'));
    }

    public function school_subscription_detail_show(Request $request, $id)
    {
        ResponseService::noPermissionThenRedirect('subscription-view');
        ...
        $sql = $this->subscription->builder()->with('subscription_bill.transaction')
            ->where('school_id', $id);
        ...
        foreach ($res as $row) {
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['invoice_id'] = $row->subscription_bill ? '#INV-' . str_pad($row->subscription_bill->id, 4, '0', STR_PAD_LEFT) : '-';
            $tempRow['date'] = format_date($row->start_date);
            $tempRow['plan'] = $row->name;

            $amount = 0;
            if ($row->subscription_bill) {
                $amount = number_format(ceil($row->subscription_bill->amount * 100) / 100, 2);
            }
            $tempRow['amount'] = $amount;

            $method = '-';
            if ($row->subscription_bill && $row->subscription_bill->transaction) {
                $method = $row->subscription_bill->transaction->payment_gateway;
            }
            $tempRow['payment_method'] = $method;
            $tempRow['status'] = $row->status;
            $tempRow['billing_type'] = $row->package_type == 0 ? 'Prepaid' : 'Postpaid';
            $tempRow['subscription_bill_id'] = $row->subscription_bill ? $row->subscription_bill->id : null;
            ...
            if ($row->status === 'Current Cycle') {
                $operate .= BootstrapTableService::menuButton(
                    ...
                    '#',
                    ['view-subscription-details'],
                    ['data-id' => $row->id, 'data-toggle' => 'modal', 'data-target' => '#view-subscription-modal']
                );
            }

            // Generate Bill — only when no bill has been generated yet
            if ($row->status === 'Bill Not Generated') {
                $operate .= BootstrapTableService::menuButton(
                    'generate_bill',
                    url('subscriptions/generate-bill', $row->id),
                    ['generate-bill'],
                    []
                );
            }

            // Receive Payment and View Receipt — when a bill exists
            if (
                in_array($row->status, ['Paid', 'Over Due', 'Failed', 'Pending', 'Unpaid', 'Bill Not Generated'])
                && $row->subscription_bill
            ) {
                if ($row->status !== 'Paid') {
                    $operate .= BootstrapTableService::menuButton(
                        'receive_payment',
                        url('subscriptions/bill-payment', $row->subscription_bill->id),
                        ['receive_payment'],
                        []
                    );
                }
                ...
                $operate .= BootstrapTableService::menuButton(
                    'receipt',
                    url('subscriptions/bill/receipt', $row->subscription_bill->id),
                    [],
                    ['target' => '_blank']
                );
            }

            // Support Cash / Support Cheque + Delete Payment — for offline payments
            if ($row->subscription_bill && $row->subscription_bill->transaction) {
                $gateway = $row->subscription_bill->transaction->payment_gateway;
                if (in_array($gateway, ['Cash', 'Cheque'])) {
                    $operate .= BootstrapTableService::menuButton(
                        $gateway === 'Cash' ? 'payment' : 'payment',
                        url('subscriptions/bill-payment', $row->subscription_bill->id),
                        ['receive_payment'],
                        []
                    );
                    $operate .= BootstrapTableService::menuDeleteButton(
                        'delete_payment',
                        url('subscriptions/bill-payment/destroy', $row->subscription_bill->payment_transaction_id)
                    );
                }
            }

            $tempRow['operate'] = $operate
                ? BootstrapTableService::menuItem($operate)
    ...
    {
        try {
            $subscription = $this->subscription->builder()
                ->with(['package', 'subscription_feature.feature', 'addons.feature'])
                ->findOrFail($id);

            $allFeatures = \App\Models\Feature::activeFeatures()->get();
            $allAddons = \App\Models\Addon::where('status', 1)->with('feature')->get();

            $subscription->setAttribute('all_features', $allFeatures);
            $subscription->setAttribute('all_addons', $allAddons);

            ResponseService::successResponse('Data fetched successfully', $subscription);
        } catch (\Throwable $th) {
            ResponseService::errorResponse($th->getMessage());
        }
    }

    public function removeSubscriptionFeature($id)
    {
        try {
            $subscriptionFeature = $this->subscriptionFeature->findById($id);
            if (!$subscriptionFeature) {
                ResponseService::errorResponse('Feature not found.');
            }

            $subscription = $this->subscription->findById($subscriptionFeature->subscription_id);
            if ($subscription->status !== 'Current Cycle') {
                ResponseService::errorResponse('Only Current Cycle features can be modified.');
            }
            $schoolId = $subscription->school_id;

            // $this->subscriptionFeature->deleteById($id);
            $subscriptionFeature = SubscriptionFeature::whereHas('feature', function ($q) {
                $q->where('is_default', 0);
            })->find($id);

            if (!$subscriptionFeature) {
                ResponseService::errorResponse('Default feature cannot be removed.');
            }
            ...
            ResponseService::successResponse('Feature removed successfully.');
        } catch (\Throwable $th) {
            ResponseService::errorResponse($th->getMessage());
        }
    }

    public function removeSubscriptionAddon($id)
    {
        try {
            $addonSubscription = $this->addonSubscription->findById($id);
            if (!$addonSubscription) {
                ResponseService::errorResponse('Addon not found.');
            }

            $subscription = $this->subscription->findById($addonSubscription->subscription_id);
            if ($subscription->status !== 'Current Cycle') {
                ResponseService::errorResponse('Only Current Cycle addons can be modified.');
            }
            $schoolId = $subscription->school_id;

            $this->addonSubscription->deleteById($id);

            // remove school cache
            $this->cache->removeSchoolCache(config('constants.CACHE.SCHOOL.FEATURES'), $schoolId);

            ResponseService::successResponse('Addon removed successfully.');
        } catch (\Throwable $th) {
            ResponseService::errorResponse($th->getMessage());
        }
    }

    public function addSubscriptionFeature(Request $request, $id)
    {
        try {
            $request->validate([
            ...
            $subscription = $this->subscription->findById($id);
            if ($subscription->status !== 'Current Cycle') {
                ResponseService::errorResponse('Only Current Cycle subscriptions can be modified.');
            }

            // Check if feature already exists
            $exists = $this->subscriptionFeature->builder()
                ->where('subscription_id', $id)
                ->where('feature_id', $request->feature_id)
                ->exists();

            if ($exists) {
                ResponseService::errorResponse('This feature is already assigned to the subscription.');
            }

            // check in addon suybscription table
            $addonSubscription = $this->addonSubscription->builder()
                ->where('subscription_id', $id)
                ->where('feature_id', $request->feature_id)
                ->exists();
            ...
            $this->subscriptionFeature->create([
                'subscription_id' => $id,
                'feature_id' => $request->feature_id
            ]);
            ...
            ResponseService::successResponse('Feature added successfully.');
        } catch (\Throwable $th) {
            ResponseService::errorResponse($th->getMessage());
        }
    }
    ...
    public function addSubscriptionAddon(Request $request, $id)
    {
        try {
            ...
            // check in subscription_features table if this feature already exists
            $exists = $this->subscriptionFeature->builder()
                ->where('subscription_id', $id)
                ->where('feature_id', $addon->feature_id)
                ->exists();
            ...
            // check if already added addons
            $assignedAddons = $this->addonSubscription->builder()
                ->where('subscription_id', $id)
                ->where('id', $request->addon_id)
                ->first();

            if ($assignedAddons) {
                ResponseService::errorResponse('This addon is already assigned to the subscription.');
            }

            // Check if addon already exists
            $exists = $this->addonSubscription->builder()
                ->where('subscription_id', $id)
                ->where('feature_id', $addon->feature_id)
                ->exists();
    ...
    public function assignPlan(Request $request)
    {
        try {
            ...
            $planId = $request->package_id;
            if (!$schoolId || !$planId) {
                ResponseService::errorResponse('School ID and Plan ID are required');
            }

            $subscriptionService = app(SubscriptionService::class);
            $subscriptionService->createSubscription($planId, $schoolId, null, true);

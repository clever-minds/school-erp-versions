...
class SystemSettingsController extends Controller
{
    ...
    public function appSettingsIndex()
    {
        ...
        $names = array('app_link', 'ios_app_link', 'app_version', 'ios_app_version', 'force_app_update', 'app_maintenance', 'student_web_maintenance', 'teacher_app_link', 'teacher_ios_app_link', 'teacher_app_version', 'teacher_ios_app_version', 'teacher_force_app_update', 'teacher_app_maintenance');
    ...
    public function verifyEmailConfiguration(Request $request)
    {
        ...
        try {
            $data_email = [
                ...
                'app_name' => $this->cache->getSystemSettings()['system_name'] ?? config('app.name'),
            ];
            ...
            Mail::send('emails.smtp_test', $data_email, static function ($message) use ($data_email, $admin_mail) {
                $message->to($data_email['email'])->subject('SMTP Configuration Verified Successfully');
                $message->from($admin_mail, $data_email['app_name']);
            });
    ...
    public function subscription_settings_update(Request $request)
    {
        ...
        $settings = array(
            'additional_billing_days',
            'current_plan_expiry_warning_days',
            'trial_days',
            'student_limit',
            'staff_limit'
        );
    ...
    public function health()
    {
        ...
        $dbCheck = $systemSettings['server_config_db_checkMark'] ?? 0;
        $queueCheck = $systemSettings['server_config_queue_checkMark'] ?? 0;
        $reverbCheck = $systemSettings['server_config_reverb_checkMark'] ?? 0;
        ...
        return view('settings.health', compact('dbCheck', 'queueCheck', 'reverbCheck', 'checksPassedCount', 'data'));
    }
}

...
class ExpenseController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('expense.index', compact('expenseCategory','sessionYear','current_session_year'));
    }
    ...
    public function show($id)
    {
        ...
        $session_year = app(CachingService::class)->getDefaultSessionYear();
        $sql = $this->expense->builder()->with('category')->where('session_year_id', $session_year->id)->select('*', DB::raw('SUM(amount) as total_salary'))->groupBy('month', 'date')
        ...
        foreach ($res as $row) {
            ...
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['amount'] = number_format($row->total_salary, 2);
            // category.name
            if ($row->staff_id) {
    ...
    public function edit($id)
    {
        //
        ResponseService::noFeatureThenRedirect('Expense Management');
        ResponseService::noPermissionThenRedirect('expense-edit');
    }

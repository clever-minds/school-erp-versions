...
class OnlineExamQuestionController extends Controller {
    ...
    public function show() {
        ...
        ->with(['online_exam_question_commons' => function($query) {
           $query->with('class_subject.subject');
           $query->with('class_section.class.medium', 'class_section.section', 'class_section.medium');
       }])
        ...
        foreach ($res as $row) {
            ...
            $tempRow['subject_name'] = $row->online_exam_question_commons->first()->class_subject->subject->name ?? '';
    ...
    public function edit($id) {
        ...
        // $onlineExamQuestion = $this->onlineExamQuestion->findById($id, ['*'], ['options', 'online_exam_question_commons.class_section', 'online_exam_question_commons.class_subject.subject']);

        $onlineExamQuestion = $this->onlineExamQuestion->builder()->where('id', $id)->with('options')
        ->with(['online_exam_question_commons' => function($query) {
            $query->with('class_subject.subject')
        ...
        // $onlineExamQuestionCommons = OnlineExamQuestionCommon::where('online_exam_question_id', $id)->with('class_section')->get()->pluck('class_section_with_medium', 'class_section_id')->toArray();
        $onlineExamQuestionCommons = $onlineExamQuestion->online_exam_question_commons->pluck('class_section_with_medium', 'class_section_id')->toArray();
        return response(view('online_exam.edit_class_questions', compact('onlineExamQuestion', 'onlineExamQuestionCommons')));
    }
    ...
    public function createBulkQuestions() {
        ...
        $classSections = $this->classSection->builder()->with('class.medium', 'class.stream', 'section', 'medium')->get();
        return view('online_exam.add_bulk_questions', compact('classSections', 'subjectTeachers','classSubjects'));
    }
    ...
    public function storeBulkData(Request $request) {
        ...
        try {
            Excel::import(new OnlineExamQuestionsImport($request->class_section_id, $request->subject_id), $request->file);
            ResponseService::successResponse('Data Stored Successfully');
        } catch (Throwable $e) {

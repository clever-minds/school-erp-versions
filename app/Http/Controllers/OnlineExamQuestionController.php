...
class OnlineExamQuestionController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->hasRole('School Admin')) {
            ...
            $subjectTeachers = $this->subjectTeacher->builder()
                ->where('session_year_id', $sessionYearId)
                ->with('subject', 'class_section')->get();
        }

        //dd($subjectTeachers->toArray());
        $classes = $this->classSchool->builder()->with('medium', 'shift', 'stream')->get();
    ...
    public function show()
    {
        ...
        $sql = $this->onlineExamQuestion->builder()->with('options', 'class.medium', 'class.shift', 'class.stream', 'class_subject.subject')
            ->whereHas('class_subject', function ($q) use ($sessionYearId) {
        ...
        foreach ($res as $row) {
            ...
            $operate = BootstrapTableService::button('fa fa-edit', route('online-exam-question.edit', $row->id), ['btn-action-edit'], ['title' => 'Edit']); // Timetable Button
            ...
            $tempRow['no'] = $no++;
            $tempRow['class_name'] = $row->class_with_medium_and_shift_and_stream ?? '';

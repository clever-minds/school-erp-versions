...
class TransportationRequestController extends Controller
{
        ...
        foreach ($res as $row) {
            ...
            if (!$row->user->hasrole('Student')) {
                $operate .= BootstrapTableService::button(
                    ...
                    ['btn', 'btn-xs', 'btn-action-hard-delete', 'btn-icon', 'cancel-service'],
                    ['data-id' => $row->id, 'title' => trans('end_transportation_service')]
                );
            } else {
                $operate .= BootstrapTableService::button('fa fa-file-pdf', route('transportation-requests.fee-receipt', $row->id), ['btn', 'btn-xs', 'btn-action-view', 'btn-icon', 'generate-paid-fees-pdf'], ['target' => "_blank", 'data-id' => $row->id, 'title' => trans('generate_pdf') . ' ' . trans('fees')]);
            }
    ...
    public function feeReceipt($id)
    {
        ...
        try {
            ...
            $user = $this->user->builder()->select('id', 'first_name', 'last_name', 'email', 'mobile')
                ->with([
                    'student' => function ($query) {

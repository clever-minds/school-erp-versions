...
class ClassSectionController extends Controller
{
    ...
    public function index()
    {
        ...
        $classes = $this->class->all(['id', 'name', 'medium_id', 'shift_id', 'stream_id'], ['stream', 'medium', 'shift']);
    ...
    public function show(Request $request)
    {
        ...
        $sql = $this->classSection->builder()->with([
            ...
            'section',
            'medium',
            'class_teachers' => function ($q) use ($sessionYearId) {
                $q->where('session_year_id', $sessionYearId)->with('teacher');
            },
            'subject_teachers' => function ($q) use ($sessionYearId) {
                $q->where('session_year_id', $sessionYearId)->with('teacher')
                    ->has('class_subject')->with([
        ...
        foreach ($res as $row) {
            ...
            if ($request->show_deleted) {
                ...
                if (Auth::user()->can('class-section-edit')) {
                    ...
                    // Reset class teachers and subject teachers for this class section
                    $operate .= BootstrapTableService::button('fa fa-refresh', route('class-section.reset', $row->id), ['btn-action-soft-delete', 'reset-class-section-data'], ['title' => trans('Reset')]);
                    // $operate .= BootstrapTableService::resetButton(route('class-section.reset', $row->id), false);
                }
                // if (Auth::user()->can('class-section-delete')) {
    ...
    public function edit($id)
    {
        ...
        $sessionYearId = $this->cache->getSessionYear()->id;
        $classSection = $this->classSection->findById($id, ['*'], [
            'class',
            'class.stream',
            'class.shift',
            'section',
        ...
        $classSubjects = $this->classSubjects->builder()->where('class_id', $classSection->class_id)->where('session_year_id', $sessionYearId)->with([
            'subjectTeachers' => function ($q) use ($id, $sessionYearId) {
                $q->where('class_section_id', $id)->where('session_year_id', $sessionYearId)->get();
            },
            'subject'
        ])->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if (!empty($request->class_teacher_id)) {
                ...
                // Update or Insert Data in Class Teachers on the basis of Class Section ID, TeacherID and Session Year ID
                $this->classTeachers->upsert($classTeachersData, ['class_section_id', 'teacher_id', 'session_year_id'], ['created_at', 'updated_at']);
            }
            ...
            if (!empty($request->subject_teachers)) {
                ...
                // Update or Insert Data in Class Teachers on the basis of Class Section ID , TeacherID, Subject ID and Session Year ID
                $this->subjectTeachers->upsert($subjectTeachersData, ['class_section_id', 'teacher_id', 'class_subject_id', 'session_year_id'], ['subject_id']);
                ...
                */
                $noTeacherAssignedToTimetable = $this->timetable->builder()->where(['type' => 'Lecture', 'subject_teacher_id' => null, 'session_year_id' => $sessionYearId])->get();
                if (!empty($noTeacherAssignedToTimetable)) {
                    $updateTimetable = [];
                    $subjectTeachers = $this->subjectTeachers->builder()->where(['class_section_id' => $id, 'session_year_id' => $sessionYearId])->get();
                    ...
                    if (!empty($updateTimetable)) {
                        $this->timetable->upsert($updateTimetable, ['id'], ['subject_teacher_id', 'session_year_id']);
                    }
                }
            }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            // 1. Find the specific class_section record
            $classSection = ClassSection::findOrFail($id);

            // 2. Store the class_id before deleting the section
            $classId = $classSection->class_id;

            // 3. Delete the class_section record
            $classSection->delete();

            // 4. Check if the class has any other active sections left
            // using exists() is faster than using count()
            // $hasRemainingSections = ClassSection::where('class_id', $classId)->exists();

            // // 5. If no other sections exist, delete the main class record
            // if (!$hasRemainingSections) {
            //     ClassSchool::where('id', $classId)->delete();
            // }
    ...
    public function removeClassTeacher($classTeacherID, $classSectionID)
    {
        ...
        try {
            DB::beginTransaction();
            $sessionYearId = $this->cache->getSessionYear()->id;
            $this->classTeachers->builder()->where(['class_section_id' => $classSectionID, 'teacher_id' => $classTeacherID, 'session_year_id' => $sessionYearId])->delete();
            $teacher = $this->user->findById($classTeacherID);

            // Check this teacher has another class teacher is exists
            $classTeachers = $this->classTeachers->builder()->where(['teacher_id' => $classTeacherID, 'session_year_id' => $sessionYearId])->first();
            if (!$classTeachers) {
    ...
    public function removeSubjectTeacher($classSectionID, $teacherID, $subjectID)
    {
        ...
        try {
            DB::beginTransaction();
            $sessionYearId = $this->cache->getSessionYear()->id;
            $this->subjectTeachers->builder()->where(['class_section_id' => $classSectionID, 'teacher_id' => $teacherID, 'subject_id' => $subjectID, 'session_year_id' => $sessionYearId])->delete();
            $subjectTeachers = $this->subjectTeachers->builder()->where(['teacher_id' => $teacherID, 'session_year_id' => $sessionYearId])->first();
            if (!$subjectTeachers) {
    ...
    public function resetClassSectionData(Request $request, $id)
    {
        try {
            ResponseService::noPermissionThenSendJson('class-section-edit');
            DB::beginTransaction();
            ClassTeacher::where('class_section_id', $id)->delete();
            SubjectTeacher::where('class_section_id', $id)->forceDelete();
            DB::commit();
            ResponseService::successResponse('Data Reset Successfully');
        } catch (Throwable $e) {
            DB::rollBack();
            ResponseService::logErrorResponse($e);
            ResponseService::errorResponse();

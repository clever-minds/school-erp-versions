...
class LanguageController extends Controller
{
    ...
    public function show()
    {
        ...
        foreach ($res as $row) {
            $operate = "";
            $operate .= BootstrapTableService::menuEditButton('edit', route('language.update', $row->id));
            $operate .= BootstrapTableService::menuButton('download', route('language.json.file', $row->code), ['language-file'], ['title' => __('file')]);
            if ($row->code != "en") {
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('language.destroy', $row->id), false);
            }
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;

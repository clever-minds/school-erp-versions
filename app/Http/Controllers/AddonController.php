    ...
    public function show()
    {
        ...
        foreach ($res as $row) {
            ...
            if (empty($showDeleted)) {
                if ($row->status == 0) {
                    $operate .= BootstrapTableService::menuButton('active', '#', ['change-addon-status'], ['title' => __('active'), 'data-id' => $row->id]);
                } else {
                    $operate .= BootstrapTableService::menuButton('inactive', '#', ['change-addon-status'], ['title' => __('inactive'), 'data-id' => $row->id]);
                }
                $operate .= BootstrapTableService::menuEditButton('edit', route('addons.update', $row->id));
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('addons.destroy', $row->id), false);
            } else {
                $operate .= BootstrapTableService::menuRestoreButton('restore', route('addons.restore', $row->id), false);
                $operate .= BootstrapTableService::menuTrashButton('trash', route('addons.trash', $row->id), 'you_want_to_delete_this_record');
            }

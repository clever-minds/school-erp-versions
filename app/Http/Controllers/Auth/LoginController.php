    ...
    public function login(Request $request)
    {
        ...
        if ($request->code) {
            ...
            if (
                ...
                $mainDBUser = User::on('mysql')->where('id', $user->id)->first();
                ...
                if ($mainDBUser && !$mainDBUser->email_verified_at && $user->hasRole('School Admin')) {
                    // Keep the user logged in so they can complete email verification
                    // The verifiedEmail middleware will restrict access to protected routes
                    // redirect()->intended() will go to the verification link if they came from one

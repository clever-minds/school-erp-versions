...
class SchoolController extends Controller
{
    ...
    public function index()
    {
        ...
        // Collect all schools currently being provisioned (installed = 0)
        $installingSchools = $this->schoolsRepository->builder()
            ->where('installed', 0)
            ->whereNotNull('database_name')
            ->get(['id', 'name', 'progress', 'provision_step'])
            ->keyBy('id')
            ->map(function ($s) {
                $stepValue = $s->provision_step ?? 'job_started';
                // If it's already a label (legacy), find the key. Otherwise use as key.
                $stepKey = isset(School::STEPS[$stepValue]) ? $stepValue : 'job_started';

                // If the value doesn't match any key, maybe it's a legacy label?
                if (!isset(School::STEPS[$stepValue])) {
                    foreach (School::STEPS as $key => $info) {
                        if ($info['label'] === $stepValue) {
                            $stepKey = $key;
                            break;
                        }
                    }
        ...
        // Reverb connection config for the frontend
        $reverb = [
    ...
    public function store(Request $request)
    {
        ...
        // If only two parts (e.g., abcd.com) → return only first
        if (count($parts) == 2) {
            $host = $parts[0];
        }
        // If more than two parts (e.g., subdomain.abcd.com)
        else {
            $host = implode('.', array_slice($parts, 0, -1)); // remove last (.com)
        }
        ...
        try {
            ...
            $prefix = env('DB_PREFIX') ?: 'eschool_saas';
            $school_name = str_replace('.', '_', $request->school_name);
            // school name get first 10 characters
            ...
            $database_name = $prefix . '_' . $schoolData->id . '_' . strtolower(strtok($school_name, " "));
            ...
            if ($request->board_id) {
                ...
                $schoolBoard->school_id = $schoolData->id;
                $schoolBoard->sch_board_id = $request->board_id;
                $schoolBoard->save();
            }
            ...
            $this->prepareRequestDataForQueue($request);
    ...
    public function show()
    {
        ...
        $sql = $this->schoolsRepository->builder()->with('user:id,first_name,last_name,email,image,mobile,email_verified_at,two_factor_enabled', 'extra_school_details.form_field', 'school_board')->with([
        ...
        if ($board_id) {
            $sql->whereHas('school_board', function ($q) use ($board_id) {
                $q->where('sch_board_id', $board_id);
            });
        }
        ...
        foreach ($res as $row) {
            ...
            if ($showDeleted) {
                ...
                $operate .= BootstrapTableService::menuTrashButton('delete', route('schools.trash', $row->id), 'this_action_is_irreversible_and_will_permanently_delete_the_school_along_with_its_entire_database');
            } else if ($row->installed == 0) {
                // Show View Progress button + Delete for installing schools
                $operate  = BootstrapTableService::menuButton('view_progress', '#', ['view-provision-progress'], ['data-school-id' => $row->id, 'data-school-name' => $row->name]);
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('schools.destroy', $row->id));
            } else {
                $operate = BootstrapTableService::menuButton('manage_admin', "#", ['update-admin-data'], ['data-toggle' => "modal", 'data-target' => "#editAdminModal"]);

                if ($row->status == 0) {
                    $operate .= BootstrapTableService::menuButton('activate', "#", ["change-school-status"], ['data-id' => $row->id]);
                } else {
                    $operate .= BootstrapTableService::menuButton('inactive', "#", ["change-school-status"], ['data-id' => $row->id]);
                }
                $operate .= BootstrapTableService::menuEditButton('edit', route('schools.update', $row->id));

                // login with demo school
                if ($row->domain == 'demo') {
                    $operate .= BootstrapTableService::menuButton('setup_demo_school', route('schools.login', $row->id), ["login-with-school"], ['data-id' => $row->id, 'target' => '_blank']);
                }
            ...
            if ($row->installed == 0) {
                $stepValue = $row->provision_step ?? 'job_started';
                $stepKey = isset(School::STEPS[$stepValue]) ? $stepValue : 'job_started';
                ...
                $stepInfo = School::STEPS[$stepKey];
                $tempRow['step_label'] = $stepInfo['label'];
                $tempRow['progress']   = $stepInfo['progress'];
                $tempRow['step']       = $stepKey;
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->board_id) {
                $board[] = [
                    'school_id' => $school->id,
                    'sch_board_id' => $request->board_id
                ];
                SchoolBoard::upsert($board, ['school_id', 'sch_board_id'], ['sch_board_id', 'school_id']);
            } else {
                SchoolBoard::where('school_id', $school->id)->delete();
            }
    ...
    public function trash($id)
    {
        ...
        try {
            ...
            ResponseService::logErrorResponse($e, "School Controller -> trash");
    ...
    {
        ResponseService::noPermissionThenSendJson('schools-list');
        try {
            $school = School::find($id);
            if ($school) {
                $url = '';
                // login to school with using school domain
                $host = request()->getHost(); // current host
                $port = request()->getPort(); // current port (8000, 8001, etc.)

                if (in_array($host, ['localhost', '127.0.0.1'])) {
                    // Local environment
                    $url = "http://{$school->domain}.localhost:{$port}/login-with-demo/{$school->id}";
                } else {
                    // Live domain
                    $url = "https://{$school->domain}.{$host}/login-with-demo/{$school->id}";
                }

                return redirect()->to($url);
            }
        } catch (\Throwable $e) {
            ResponseService::errorResponse($e->getMessage(), "School Controller -> Login method");
        }
    }

    public function loginWithDemo($id)
    {
        try {
            $school = School::on('mysql')->find($id);
            if ($school) {
                Config::set('database.connections.school.database', $school->database_name);
                DB::purge('school');
                DB::connection('school')->reconnect();
                ...
                $user = User::find($school->admin_id);
                if ($user) {
                    Auth::login($user);
                    Session::put('super_admin_login_with_demo_school', true);
            ...
            ResponseService::errorResponse($e->getMessage(), "School Controller -> LoginWithDemo method");
        }
    }
}

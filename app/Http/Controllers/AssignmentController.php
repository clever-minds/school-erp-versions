...
class AssignmentController extends Controller
{
    ...
    public function index()
    {
        ...
        $classSections = $this->classSection->builder()->with('class', 'class.stream', 'class.shift', 'section', 'medium')->get();
        $subjectTeachers = $this->subjectTeacher->builder()->with('subject:id,name,type')->get();


        return response(view('assignment.index', compact('assignment', 'classSections', 'subjectTeachers')));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $assignmentData = [
                ...
                $request->resubmission ? $request->extra_days_for_resubmission : null,
            ...
            unset($assignmentData['subject_id'], $assignmentData['user_id']);
            ...
            if ($request->hasFile('file')) {
                $fileData = [];
                $assoc = $this->files->model()->modal()->associate($assignment);
            ...
            if ($request->add_url) {
                ...
                foreach ((array) $request->add_url as $url) {
                    $urlData[] = [
                        'modal_type' => $assoc->modal_type,
                        'modal_id' => $assoc->modal_id,
            ...
            $classSubjectsMap = $this->class_subjects
                ->builder()
                ->whereIn('id', $assignmentCommons->pluck('class_subject_id')->toArray())
                ->get()
                ->keyBy('id');
            ...
            $title = 'New assignment added in ' . $subjectName->name_with_type;
            $body = $request->name;
            $type = 'assignment';
            ...
            foreach ($students as $student) {
                ...
                $baseData = [
                    ...
                    'assigment_status_update' => 0,
                ];

                if ($student->user_id) {
                    $allPayloads = array_merge(
                        $allPayloads,
                        buildPayloads([$student->user_id], $title, $body, $type, $baseData)
                    );
                }
                ...
                if ($student->guardian_id) {
                    $allPayloads = array_merge(
                        ...
                        buildPayloads(
                            ...
                            $body,
                            $type,
                            array_merge($baseData, ['child_id' => $student->id])
                        )
                    );
                }
            }
    ...
    public function show(Request $request)
    {
        ...
        $semester_id = $this->cache->getSemester()->id ?? null;

        $sql = $this->assignment->builder()->with([
            'class_section.medium',
            'file',
            'class_subject.subject',
            'assignment_commons.class_section.class.stream',
            'assignment_commons.class_section.class.shift',
            ...
            ->when($semester_id, function ($query) use ($semester_id) {
                $query->where('semester_id', $semester_id)->orWhere('semester_id', null);
            })
        ...
        foreach ($res as $row) {
            ...
            $operate = BootstrapTableService::menuEditButton('edit', route('assignment.update', $row->id));
            $operate .= BootstrapTableService::menuButton("view_submission", route('assignment.submissionDetails', $row->id));
            $operate .= BootstrapTableService::menuDeleteButton('delete', route('assignment.destroy', $row->id));
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            ...
            foreach ($section_ids as $section_id) {
                ...
                $assignmentData = array_merge($assignmentData, [
                    'class_section_id' => $section_id
                ]);
                ...
                $getClassSubjectType = $this->class_subjects
                    ->findById($classSubjects->id, ['type']);

                if ($getClassSubjectType->type == 'Elective') {
                    $notifyUser[] = $this->studentSubject
                        ->builder()
                        ->select('student_id')
            ...
            if ($request->hasFile('file')) {
                $fileData = [];
                $assignmentModelAssociate = $this->files
                    ->model()
                    ->modal()
            ...
            if ($request->add_url) {
                ...
                $this->files->upsert(
                    $tempUrlData,
                    ['id'],
                    ['id', 'modal_type', 'modal_id', 'file_name', 'type', 'file_url']
                );
            } else {
            ...
            $title = 'Updated assignment in ' . $subjectName->name_with_type;
            $body = $request->name;
            $type = 'assignment';
    ...
    public function viewAssignmentSubmission()
    {
        ...
        $subjectTeachers = $this->subjectTeacher->builder()->where('session_year_id', $sessionYear->id)->with('subject:id,name,type', 'class_subject:id,class_id,subject_id,semester_id')->get();
    ...
    {
        $sessionYear = $this->cache->getSessionYear();
        $semesterId = $this->cache->getSemester()->id ?? null;
    ...
    public function showAssignmentSubmissionDetails($id, $class_section_id, $subject_id)
    {
        ...
        $sql = $this->assignmentSubmission->builder()->with('assignment.class_subject.subject', 'student:first_name,last_name,id,image,email', 'file', 'session_year', 'assignment.class_section.class', 'assignment.class_section.class.shift', 'assignment.class_section.medium')->where('assignment_id', $id)
    ...
    public function assignmentSubmissionList()
    {
        ...
        $sql = $this->assignmentSubmission->builder()->with(['assignment.class_subject.subject', 'student:first_name,last_name,id,image,email', 'file', 'session_year', 'assignment.class_section.class.stream', 'assignment.class_section.class.shift', 'assignment.class_section.medium'])
            //search query
            ->whereHas('assignment', function ($query) use ($sessionYear, $semesterId) {
                $query->when($semesterId, function ($query) use ($semesterId) {
                    $query->where('semester_id', $semesterId)->orWhereNull('semester_id');
                });
    ...
    public function updateAssignmentSubmission($id, Request $request)
    {
        ...
        try {
            ...
            $customData = ['assignment_id' => $assignmentData->id, 'class_subject_id' => $assignmentData->class_subject_id, "assigment_status_update" => 1];

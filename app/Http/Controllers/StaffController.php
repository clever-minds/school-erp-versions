...
class StaffController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($allowance_status) {
                $this->staffSalary->upsert($allowance_data, ['staff_id', 'payroll_setting_id', 'session_year_id'], ['amount', 'percentage']);
            }
            ...
            if ($deduction_status) {
                $this->staffSalary->upsert($deduction_data, ['staff_id', 'payroll_setting_id', 'session_year_id'], ['amount', 'percentage']);
            }
    ...
    public function show(Request $request)
    {
        ...
        $sql = $this->user->builder()
            ->where(function ($query) {
                $query->whereHas('roles', function ($q) {
                    ...
                    $q->whereNotIn('name', ['Teacher', 'Student', 'Guardian', 'School Admin', 'Super Admin']);
                });
            })
        ...
        foreach ($res as $row) {
            ...
            //     $operate .= BootstrapTableService::trashButton(route('staff.trash', $row->id), 'you_want_to_delete_this_record');
            // } else {
            ...
            //         $operate .= BootstrapTableService::button('fa fa-eye', route('staff.payroll-structure', ['id' => $row->id, 'type' => 'staff']), ['btn-gradient-warning'], ['title' => __('salary_structure')]);
            //     }
            ...
            if ($request->show_deactive == 1) {
                ...
                $operate = BootstrapTableService::menuRestoreButton('restore', route('staff.restore', $row->id), false);
                $operate .= BootstrapTableService::menuTrashButton('trash', route('staff.trash', $row->id), 'you_want_to_delete_this_record');
            } else {
                //Show Edit and Soft Delete Buttons
                $operate = BootstrapTableService::menuEditButton('edit', route('staff.update', $row->id));
                if (app(FeaturesService::class)->hasFeature('Expense Management')) {
                    $operate .= BootstrapTableService::menuButton('salary_structure', route('staff.payroll-structure', ['id' => $row->id, 'type' => 'staff']));
                }
                $operate .= BootstrapTableService::menuDeleteButton('delete', route('staff.destroy', $row->id), 'you_want_to_delete_this_record');
                // $operate .= BootstrapTableService::button('fa fa-exclamation-triangle', route('staff.destroy', $row->id), ['deactivate-staff', 'btn-gradient-info'], ['title' => __('inactive')]);
            }
    ...
    public function staff_id_card_list()
    {
        ...
        $sql = $this->user->builder()->where(function ($query) {
            $query->whereHas('roles', function ($q) {
                $q->where('custom_role', 1);
            })->orWhereHas('roles', function ($q) {
    ...
    public function viewSalaryStructure($id, $type = 'teacher')
    {
        ResponseService::noAnyPermissionThenRedirect(['staff-list', 'teacher-list']);

        $sessionYear = $this->cache->getSessionYear();
        $user = $this->user->builder()->with([
            'staff' => function ($q) use ($sessionYear) {
                $q->withCount([
                    'staffSalary as allowanceCounter' => function ($q) use ($sessionYear) {
                        $q->where(function ($q) use ($sessionYear) {
                            $q->where('session_year_id', $sessionYear->id)->orWhereHas('payrollSetting', function ($q) {
                                $q->where('name', 'Transportation Deduction');
                            });
                        })->whereHas('payrollSetting', function ($q) {
                            $q->where('type', 'allowance');
                        });
                    }
                ])
                    ->withCount([
                        'staffSalary as deductionCounter' => function ($q) use ($sessionYear) {
                            $q->where(function ($q) use ($sessionYear) {
                                $q->where('session_year_id', $sessionYear->id)->orWhereHas('payrollSetting', function ($q) {
                                    $q->where('name', 'Transportation Deduction');
                                });
                    ...
                    ->with(['staffSalary' => function ($q) use ($sessionYear) {
                        $q->where(function ($q) use ($sessionYear) {
                            $q->where('session_year_id', $sessionYear->id)->orWhereHas('payrollSetting', function ($q) {
                                $q->where('name', 'Transportation Deduction');
                            });
        ...
        $payrollSettings = $this->payrollSetting
            ->builder()
            ->withTrashed()
            ->get();

        $allowances = $payrollSettings->where('type', 'allowance')->values();
        $deductions = $payrollSettings->where('type', 'deduction')->values();
    ...
    public function deletePayrollSetting($id)
    {
        ...
        try {
            ...
            $staffSalary = $this->staffSalary->builder()
                ...
                ->first();
            if ($staffSalary) {
                return ResponseService::errorResponse("Transportation Deduction cannot be deleted");
            }
    ...
    {
        $staffTemplates = CertificateTemplate::where('type', 'Staff')
            ->pluck('name', 'id');
        return view('staff.my_certificate', compact('staffTemplates'));
    }

    public function showMyCertificates(Request $request)
    {
        ResponseService::noFeatureThenRedirect('ID Card - Certificate Generation');
        ...
        foreach ($res as $row) {
            ...
            $operate = '';
            $operate = BootstrapTableService::button('fa fa-eye', route('staff.my-certificate.view', $row->id), ['btn-action-view'], ['title' => 'View', 'target' => '_blank']);
    ...
    public function viewMyCertificate($id)
    {
        ResponseService::noFeatureThenRedirect('ID Card - Certificate Generation');

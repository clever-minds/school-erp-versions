...
class OnlineExamQuestion extends Model {
    ...
    protected $appends = ['class_with_medium', 'class_with_medium_and_shift', 'subject_with_name', 'class_with_medium_and_shift_and_stream'];
    ...
    public function scopeOwner($query)
    {
        if (Auth::user()) {
            ...
            if (Auth::user()->hasRole('School Admin') || Auth::user()->hasRole('Staff')) {
                $sessionYearId = app(CachingService::class)->getSessionYear()->id;
                return $query->where('school_id', Auth::user()->school_id)->whereHas('class_subject', function($q) use($sessionYearId) {
    ...
    public function getClassWithMediumAndShiftAndStreamAttribute() {
        if ($this->relationLoaded('class') && $this->class) {
            $name = $this->class->name;
            if ($this->class->relationLoaded('medium') && $this->class->medium) {
                $name .= ' - ' . $this->class->medium->name;
            }
            if ($this->class->relationLoaded('stream') && $this->class->stream) {

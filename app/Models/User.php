...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        return $this->hasOne(Students::class, 'user_id', 'id')->withTrashed()->owner();
    }

    public function promote_student()
    {
        return $this->hasMany(PromoteStudent::class, 'student_id', 'id');
    }
    public function studentWithoutOwner()
    {
    ...
    {
        return $this->hasOne(Students::class, 'user_id', 'id')->owner();
    }
        ...
        if (Auth::user()) {
            if (Auth::user()->school_id) {
                ...
                if (Auth::user()->hasRole('Super Admin')) {
                    ...
                    $query->where('school_id', $studentAuth->school_id);
                } else {
                    $query->where('school_id', null);
                }
            }

            // Apply joining date filter for staff-related users (History-Awareness)
            if (!Auth::user()->hasRole('Super Admin')) {
                ...
                if ($sessionYear) {
                    // Negative Exclusion Strategy (Recursion Safe)
                    // Hide anyone who HAS a staff record but the joining date is in the future.
                    $query->whereDoesntHave('staff', function ($sq) use ($sessionYear) {
                        $sq->withoutGlobalScopes()->where('joining_date', '>', $sessionYear->original_end_date);
                    });
                }
            }
        }
    ...
    {
        return $this->hasMany(Students::class, 'guardian_id')->withTrashed()->owner();
    }
    ...
    public function teacher()
    {
        ...
        return $this->hasOne(Staff::class, 'user_id', 'id')->withoutGlobalScopes();
    }
